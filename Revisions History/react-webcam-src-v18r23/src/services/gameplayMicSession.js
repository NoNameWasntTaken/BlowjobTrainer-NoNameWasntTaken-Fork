...
async function startGameplayMicInternal() {
    ...
    try {
        ...
        // Browsers may re-suspend between the initial resume and getUserMedia completion.
        if (ctx.state !== 'running') await ctx.resume()

        // Play a short silent buffer to force the OS audio output pipeline to settle.
        // getUserMedia with echoCancellation triggers an async output device reconfiguration;
        // rendering silence through ctx.destination completes that before real audio plays.
        await new Promise((resolve) => {
            try {
                const settleLen = Math.ceil(ctx.sampleRate * 0.3)
...
/**
 * Re-opens the gameplay mic after navigating away from Play (e.g. Audio tab) or if the stream ended.
 * No-op when already active and connected. Throws on the same failures as {@link startGameplayMic}.
 */
export async function ensureGameplayMicBeforeResume() {
    if (audioProcessingService.getMicConnected() && isGameplayMicActive()) {
        return
    }
    ...
    if (!isGameplayMicActive() || !audioProcessingService.getMicConnected()) {
        throw new Error('Microphone could not be restarted for gameplay')
    }
}
