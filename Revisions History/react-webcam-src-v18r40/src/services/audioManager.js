...
import { CUSTOM_VOICE_CATEGORY_COUNT, CUSTOM_VOICE_SLOT_KEYS } from '../constants/customVoiceCategories'
...
export const audioManager = {
    ...
    importPack: async (zipFile, options = {}) => {
        ...
        const migratedAudioFiles = migrateAudioPackFiles(processedAudioFiles)
        ...
        const CUSTOM_ALLOWED_KEYS = new Set(CUSTOM_VOICE_SLOT_KEYS)
            ...
            if (cat === 'Custom') {
                ...
                packKeys.forEach(key => {
                    if (!CUSTOM_ALLOWED_KEYS.has(key)) {
                        warnings.push(
                            `Invalid Custom key "${key}" (allowed: Custom1–Custom${CUSTOM_VOICE_CATEGORY_COUNT})`
                        )
                    }
                })
            } else if (AUDIO[cat]) {
