...
import { computeSessionEndRank } from '../utils/sessionEndRank';

/**
 * Map session-end rank to JSON export string.
 * @param {string} completionStatus
 * @param {number} rank - Rank.* numeric
 * @returns {'failed'|'pass'|'good'|'perfect'|'none'}
 */
export function performanceRankToExportString(completionStatus, rank) {
...
export function formatSessionData(currentLevel, playTime, completionStatus) {
    ...
    const resolvedStatus = completionStatus || 'completed';
    const rankNumeric = computeSessionEndRank(currentLevel);
    ...
    return {
        ...
        performanceRank: performanceRankToExportString(resolvedStatus, rankNumeric),
