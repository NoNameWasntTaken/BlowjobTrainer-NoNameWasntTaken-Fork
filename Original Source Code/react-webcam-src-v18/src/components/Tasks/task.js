import { getRandomInt, getRandomElement } from "../randomInt";
...
export function createRESTTask(options = {}) {
    ...
    if (!options.audio) {
        options.audio = options.useBall ? AUDIO.Task.REST_BALL : AUDIO.Task.REST
    }
...
export function createFINISHTask(options = {}) {
    ...
    // Select audio event based on rest type
    if (!options.audio) {
        options.audio = AUDIO.Finish.CLEAN
    }
    return {
        ...options,
...
export function createHOLDTask(options = {}) {
    ...
    if (!options.audio) {
        options.audio = AUDIO.Hold[wordKey] || AUDIO.Hold.ONE
    }

    // Add halfway audio based on depth
    options.audioHalfway = AUDIO.Hold[`${wordKey}_HALF`] || AUDIO.Hold.ONE_HALF

    // Add three-quarter audio based on depth for long holds
    options.audioThreeQuarter = AUDIO.Hold[`${wordKey}_3Q`] || AUDIO.Hold.ONE_3Q
...
export function createHITTask(options = {}) {
    ...
    if (!options.audio) {
        ...
        const wordKey = depthWords[options.targetDepth] || 'ONE'
        options.audio = AUDIO.Hit[wordKey] || AUDIO.Hit.FOUR
    }
...
export function createUPANDDOWNTask(options = {}) {
    ...
    if (!options.audio) {
        ...
        options.audio = AUDIO.UpDown[key] || AUDIO.UpDown.MED
    }
...
export function createCLAPTask(options = {}) {
    ...
    // Select audio event
    if (!options.audio) {
        options.audio = AUDIO.Clap.FACE // we'll update this when we add clap audio
    }
...
export function createENDLESSTask(options = {}) {
    ...
    // Select audio event
    if (!options.audio) {
        options.audio = AUDIO.Level.START_00 // we'll update this when we add clap audio
    }
...
export const calculateLevelDuration = (tasks, tenSecond = false) => {
    ...
    const totalSeconds = tasks.reduce((sum, task) => {
        ...
        const taskDuration = task.timeLimit || task.time || 0
...
export const getTaskSummary = (task) => {
    switch (task.type) {
        ...
        case TaskType.REST:
            return `Rest for ${task.timeLimit}s`
        case TaskType.CLAP:
