...
export const GRACE_MAX = 20
export const GRACE_RATE = 0.1 // seconds per 10 points earned

// Consistency bonuses
export const RHYTHM_BONUS_MAX = 0.25
export const DEPTH_BONUS_MAX = 0.2
...
export function computeRhythmBonusMultiplier(diveWindow) {
    ...
    const tempos = diveWindow.map(d => d.tempo).filter(t => typeof t === 'number')
...
export function computeDepthBonus(diveWindow) {
    ...
    const allMatch = diveWindow.every(d => {
        ...
        return dMin === minDepth && dMax === maxDepth
    })
