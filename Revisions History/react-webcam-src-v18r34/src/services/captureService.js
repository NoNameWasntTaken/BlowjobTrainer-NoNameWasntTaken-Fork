...
/** Task types hardcoded to never signal video captures (mirrors TASK_TYPES_NO_VIDEO in useCaptureManager). */
const TASK_TYPES_NO_VIDEO = new Set([TaskType.HITDEPTH])
/** Task types hardcoded to never signal photo captures (mirrors TASK_TYPES_NO_PHOTO in useCaptureManager). */
...
export function getLevelCaptureChannelAvailability(profile, level) {
    ...
    const photoPossible =
        ...
        tasks.some((t) => !TASK_TYPES_NO_PHOTO.has(t?.type) && normalizeTaskCapture(t).allowPhotos === true)
    const videoPossible =
        (Number(lvl.videoCaptureLimit) || 0) >= 1 &&
        tasks.some((t) => !TASK_TYPES_NO_VIDEO.has(t?.type) && normalizeTaskCapture(t).allowVideos === true)
