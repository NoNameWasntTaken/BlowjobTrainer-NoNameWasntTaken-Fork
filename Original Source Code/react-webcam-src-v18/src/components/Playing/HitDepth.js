...
function HitDepth({ task, onTaskOver }) {
    ...
    const resetHitState = useCallback(() => {
        ...
        setInstruction(`Hit depth ${task?.targetDepth}`)
    }, [task?.targetDepth])
    ...
    useEffect(() => {
        ...
        // Reset for next attempt when we move one level shallower than target
    ...
    }, [diveState.current, task.targetDepth, hitState, hitSuccess, hitFailed, paused, setInstruction, hasStarted])
    ...
    }, [diveState.current, hitState, hasStarted, setFeedback, task.targetDepth, stopVibration])
