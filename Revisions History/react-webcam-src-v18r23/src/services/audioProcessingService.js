...
import {
    isSpeechTapWanted,
    setSpeechWantsTap,
    setSpeakPcmChunkHandler,
    speakPcmChunkHandlerRef,
} from './speakPcmTapSession'
...
/** True after micSource → speakPcmTapNode connect; cleared when that edge is torn down. */
let speakPcmTapMicSourceConnected = false
...
/** In-flight speak PCM worklet preload (shared with connectSpeakPcmTap via speakPcmTapWorkletLoaded). */
let speakPcmTapWorkletPreloadPromise = null
...
export const audioProcessingService = {
    ...
    /**
     * Ensures the shared AudioContext is running (not suspended). Browsers often start the context
     * suspended until a user gesture; call from pointer/key handlers after ensureAudioContext.
     * @returns {Promise<void>}
     */
    async resumeAudioContextIfSuspended() {
    ...
    /**
     * Load public/speak-pcm-tap.js into the AudioContext early (e.g. on app mount) so the first
     * Speak task or speech calibration does not pay addModule latency before PCM reaches Sherpa.
     * No-op if already loaded. Safe to call before connectMic.
     */
    async preloadSpeakPcmTapWorklet() {
        if (typeof window === 'undefined') return
        if (speakPcmTapWorkletLoaded) return
        if (!speakPcmTapWorkletPreloadPromise) {
            speakPcmTapWorkletPreloadPromise = (async () => {
                ...
                speakPcmTapWorkletPreloadPromise = null
            })
        }
        return speakPcmTapWorkletPreloadPromise
    },
    ...
    /**
     * Creates or reuses speak-pcm-tap and wires mic → tap when {@link isSpeechTapWanted} and mic is live.
     * Port handler reads {@link speakPcmChunkHandlerRef} for lazy decode. No-op when speech not armed.
     * @returns {Promise<void>}
     */
    async ensureSpeakPcmTapIfWanted() {
        if (!isSpeechTapWanted()) {
            audioProcessingService.disconnectSpeakPcmTap()
            return
        }
        if (!audioContext || !lmsNode) {
            throw new Error('audioProcessingService.ensureSpeakPcmTapIfWanted: ensureAudioContext must be called first')
        }
        ...
        if (!speakPcmTapNode) {
            ...
            tap.port.onmessage = (ev) => {
                if (!isSpeechTapWanted()) return
                if (!ev.data?.pcm || typeof ev.data.rms !== 'number') return
                const h = speakPcmChunkHandlerRef.current
                if (!h) return
                try {
                    h(ev.data)
                } catch (e) {
        ...
        if (micSource && speakPcmTapNode && !speakPcmTapMicSourceConnected) {
            try {
                micSource.connect(speakPcmTapNode)
                speakPcmTapMicSourceConnected = true
            } catch (e) {
    ...
    /**
     * Mic (pre-LMS) → PCM tap worklet → silent gain. Prefer {@link ensureSpeakPcmTapIfWanted} + speakPcmTapSession.
     * @param {(data: { pcm: Float32Array, rms: number }) => void} onChunk
     * @deprecated Use setSpeakPcmChunkHandler + ensureSpeakPcmTapIfWanted
     */
    async connectSpeakPcmTap(onChunk) {
        setSpeechWantsTap(true)
        setSpeakPcmChunkHandler(onChunk)
        await audioProcessingService.ensureSpeakPcmTapIfWanted()
    },

    disconnectSpeakPcmTap() {
        speakPcmTapMicSourceConnected = false
        if (speakPcmTapNode) {
            try {
