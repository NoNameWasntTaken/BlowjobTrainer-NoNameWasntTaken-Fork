 ...
 * @param {React.ReactNode} summaryExtra - Optional slot after Preview (e.g. Summary/Custom toggle)
 * @param {boolean} omitLabel - When true, do not render the label column (e.g. hold voice mode row supplies its own menu)
 * @param {object} selectStyle - Optional inline styles for the select element (non-summary); merged after boxSizing
 */
const AudioSelector = ({ value, onChange, label = "Voice Line", allowedOptions = null, audioPackId = null, summaryStyle = false, summaryExtra = null, omitLabel = false, selectStyle: selectStyleProp = undefined }) => {
    ...
    React.useEffect(() => {
        ...
        if (customPack && customPack.audioFiles) {
            for (const category in customPack.audioFiles) {
                const keys = Object.keys(customPack.audioFiles[category] || {})
    ...
    const selectStyle = summaryStyle
        ...
        : selectStyleProp != null
            ? { boxSizing: 'border-box', ...selectStyleProp }
            : undefined

    const previewButton = value ? (
        <button
            type="button"
            onClick={handlePreview}
            className={summaryStyle ? 'task-audio-preview task-audio-preview-summary' : 'task-audio-preview'}
    ...
    return (
        <div className={rootClassName} style={rootStyle}>
            {!omitLabel && (
                <div className={summaryStyle ? 'summary-voice-label' : undefined}>
                    {label}:
                </div>
            )}
            <select
            ...
            {summaryStyle ? (
                <div className="summary-voice-actions">
                    {previewButton}
                    {summaryExtra}
                </div>
            ) : (
