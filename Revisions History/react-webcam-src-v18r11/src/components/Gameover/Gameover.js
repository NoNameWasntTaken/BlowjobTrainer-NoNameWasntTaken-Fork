...
function Gameover() {
    ...
    const getTotalDivesForDepth = useCallback((depth) => {
        ...
        return currentLevel.summaries.reduce((total, summary) => {
            ...
            if ((summary.type === TaskType.UPANDDOWN || summary.type === TaskType.HITDEPTH) &&
                ...
                return total + (summary.dives || summary.counts?.pass || 0);
            }
            return total;
        }, 0);
    }, [currentLevel]);
    ...
    useEffect(() => {
        ...
        //  -- UPDATE PLAYER STATS --
        // Guard: only process stats once per level completion to prevent inflated attempts count
        ...
        setTimeStats(prev => {
            ...
            return {
                ...prev,
                totalPlayTime: prev.totalPlayTime + playTimeValue,
                holdTimeByDepth: newHoldTimeByDepth
            }
        })

        // Check time-based achievements
        checkTrainingDedication(playTimeValue)
        ...
        setDiveStats(prev => ({
            ...
            perfectTaskExecutions: {
                ...
                HOLDANDCLAP: prev.perfectTaskExecutions.HOLDANDCLAP + getPerfectTaskCount(TaskType.HOLDANDCLAP),
            }
        }))
        ...
        // useAchievements callbacks excluded - they are stable in practice; including them causes
        // re-run loops. Stats update is guarded by hasProcessedCompletionRef.
    ...
    const getTotalHoldTime = () => {
        ...
        for (const summary of currentLevel.summaries) {
            if (summary.type === 'hold' || summary.type === TaskType.HOLDANDCLAP) {
            ...
            <div className="margin-y-lg card" >
                ...
                {currentLevel?.summaries?.map((summary, index) => (
                    <div key={index} className="margin-y-sm" style={{ borderBottom: '1px solid #eee', padding: '5px 0' }}>
                        ...
                        {summary.type !== TaskType.CLAP && summary.type !== TaskType.HOLDANDCLAP && summary.type !== TaskType.ENDLESS && <div style={{ fontSize: '0.9em', opacity: 0.8, marginTop: '3px' }}>
