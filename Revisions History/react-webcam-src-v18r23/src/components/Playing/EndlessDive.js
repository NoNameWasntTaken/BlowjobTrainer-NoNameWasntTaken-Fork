...
function EndlessDive({ task, onTaskOver, priorScore = 0 }) {
    ...
    useEffect(() => {
        if (motion.dir === 'up') {
            ...
            const sumOfWindow = window.reduce((s, dive) => {
                const v = dive.score;
                return s + (typeof v === 'number' && Number.isFinite(v) ? v : 0);
            }, 0);
            const rhythmConsistencyMult = computeRhythmBonusMultiplier(window);
            const depthConsistencyMult = computeDepthBonus(window);
            const nextRhythm = sumOfWindow * RHYTHM_BONUS_MAX * rhythmConsistencyMult;
            const nextDepth = sumOfWindow * depthConsistencyMult;
            rhythmBonusRef.current = Number.isFinite(nextRhythm) ? nextRhythm : 0;
            depthBonusRef.current = Number.isFinite(nextDepth) ? nextDepth : 0;
            allScoresRef.current = [...allScoresRef.current, movement];
            setAllScoresRef.current(allScoresRef.current);
            const newTotals = {
                hold: totalHoldScoreRef.current,
