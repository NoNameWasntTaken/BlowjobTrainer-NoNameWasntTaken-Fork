...
import { cameraVideoDeviceIdAtom } from '../../atoms/cameraAtom'
...
function WebcamDisplay({ webcamRef, canvasRef, canvasDimensions, scaleFactor, fps, showCameraToggle }) {
    ...
    useLayoutEffect(() => {
        ...
        const update = () => {
            ...
            const maxH = window.innerHeight * VIEWPORT_HEIGHT_CAP
    ...
    }, [canvasDimensions.width, canvasDimensions.height, isSideways])
    ...
    // `maxWidth`/`maxHeight` are neutralized because legacy global CSS in
    // App.css (`#video-container video { max-width: 100%; ... }` and
    // `canvas { max-width: 100% }`) otherwise caps the pre-rotation width to
