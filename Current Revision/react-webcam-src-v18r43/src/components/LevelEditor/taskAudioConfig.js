...
import { getAudioMode, getCustomAudioOptions as customCueOptions } from '../../services/customAudio'

/**
 * Get Custom.* audio options from pack (only lines that have files).
 * When audioPackId is null, falls back to currently active pack (Use Selected Audio).
 */
export function getCustomAudioOptions(audioPackId) {
    const packId = audioPackId ?? audioManager.getActivePackId()
    const pack = packId ? audioManager.loadPack(packId) : null
    if (!pack) return []
    return customCueOptions(pack)
}
...
/** Task types whose legacy mixed list also offered release cues (score-event selects). */
const TASK_TYPES_WITH_RELEASE = new Set([
    'hold',
    'updown',
    'hitdepth',
    'clap',
...
function taskSpecificAudio(taskType) {
    switch (taskType) {
        case 'hold':
            return HOLD_AUDIO
        case 'updown':
 ...
 */
export function getAudioOptionsForTaskType(taskType, showCustomVoiceLines = false, audioPackId = null) {
    let baseOptions = taskSpecificAudio(taskType)
    if (TASK_TYPES_WITH_RELEASE.has(taskType)) {
        baseOptions = [...baseOptions, ...RELEASE_AUDIO]
    }
    if (showCustomVoiceLines) {
        return [...baseOptions, ...getCustomAudioOptions(audioPackId)]
    }
    return baseOptions
}

/**
 * Cue list for one voice-category dropdown.
 * category: 'standard' | 'release' | 'custom'
 * holdSlot: 'main' | 'halfway' | '3q' (halfway and 3Q have no release list)
 */
export function getVoiceLineOptions(taskType, category, audioPackId = null, holdSlot = 'main') {
    if (category === 'release') return RELEASE_AUDIO
    if (category === 'custom') return getCustomAudioOptions(audioPackId)
    if (holdSlot === 'halfway') return HOLD_PROGRESS_HALF_AUDIO
    if (holdSlot === '3q') return HOLD_PROGRESS_3Q_AUDIO
    return taskSpecificAudio(taskType)
}

/** Keep a saved cue visible when it is outside the filtered category list. */
export function withSavedVoiceOption(options, value) {
    if (!value || options.some((option) => option.value === value)) return options
    return [...options, { value, label: `${value} (not available in this list)` }]
}
...
export function getHoldProgressHalfAudioOptions(showCustomVoiceLines = false, audioPackId = null) {
    if (showCustomVoiceLines) {
        return [...HOLD_PROGRESS_HALF_AUDIO, ...getCustomAudioOptions(audioPackId)]
    }
    return HOLD_PROGRESS_HALF_AUDIO
}

export function getHoldProgress3QAudioOptions(showCustomVoiceLines = false, audioPackId = null) {
    if (showCustomVoiceLines) {
        return [...HOLD_PROGRESS_3Q_AUDIO, ...getCustomAudioOptions(audioPackId)]
    }
...
export function getDefaultUpAndDownAudio(minDepth, maxDepth, tempo) {
    ...
    // Try specific range first (e.g., ONE_TWO_SLOW, ONE_THREE_MED)
    const specificKey = `${startWord}_${endWord}_${tempoWord}`
    if (AUDIO.UpDown[specificKey]) {
        return `UpDown.${specificKey}`
    }

    // Fallback to generic tempo
    const genericTempo = tempoWord
...
/** Automatic task-specific cue for the current task settings. */
export function getDefaultTaskAudio(task) {
    switch (task.type) {
        case 'hold':
            return getDefaultHoldAudio(task.targetDepth || 1)
        case 'updown':
            return getDefaultUpAndDownAudio(task.minDepth || 1, task.maxDepth || 2, task.tempo || 30)
        case 'hitdepth':
            return getDefaultHitAudio(task.targetDepth || 4)
        case 'holdandclap':
            return getDefaultHoldAndClapAudio(task.targetDepth || 1)
        case 'rest':
            return 'Rest.REST'
        case 'rest ball':
            return 'Rest.REST_BALL'
        case 'speak':
            return 'Speak.Speak1'
        case 'endless':
            return 'Endless.ENDLESS'
        case 'finish':
            return 'Finish.CLEAN'
        default:
            return taskSpecificAudio(task.type)[0]?.value || null
    }
}

/**
 * Displayed category for one cue slot. Does not write the task.
 * Halfway and 3Q never display Release.
 */
export function voiceCategoryForSlot(task, slot) {
    if (slot === 'halfway' || slot === '3q') {
        const mode = slot === 'halfway' ? task.audioHalfwayMode : task.audioThreeQuarterMode
        const audio = slot === 'halfway' ? task.audioHalfway : task.audioThreeQuarter
        if (mode === 'standard') return 'standard'
        if (mode === 'custom' || /^custom\./i.test(audio || '')) return 'custom'
        return 'standard'
    }
    return getAudioMode(task)
}
...
/**
 * Cue to apply when switching a summary row between Summary and Custom.
 * Summary → that rank's editor default Rank.* line. Custom → first Custom.* cue, or None.
 */
export function getSummaryModeDefaultCue(rankKey, useCustom, audioPackId) {
    if (useCustom) {
        return getCustomAudioOptions(audioPackId)[0]?.value ?? null
    }
    return DEFAULT_EDITOR_SUMMARY_AUDIO[rankKey] ?? null
}

/**
 * Options for one summary row. Summary = default Rank.* list; Custom = pack Custom.* only.
 * A saved cue outside the active list stays visible as "(not available in this list)".
 * @param {object} summaryAudioShowCustom - e.g. { failed: true }; true means that row is on Custom
 */
export function buildSummaryAudioRowOptions(summaryAudio, summaryAudioShowCustom, rankKey, audioPackId) {
    const value = typeof summaryAudio?.[rankKey] === 'string' ? summaryAudio[rankKey] : ''
    if (summaryAudioShowCustom?.[rankKey]) {
        return withSavedVoiceOption(getCustomAudioOptions(audioPackId), value)
    }
    return withSavedVoiceOption(DEFAULT_LEVEL_SUMMARY_AUDIO_OPTIONS, value)
}
