...
function validateCLIArguments(config) {
    ...
    // --auto-start requires --level (validated after mutual-exclusion rules above)
    if (config.autoStart && !config.level) {
        errors.push('--auto-start requires --level');
    }
    ...
    if (config.enableCaptures && !config.level) {
        errors.push('--enable-captures requires --level');
    }
...
ipcMain.handle('show-directory-picker', async (event, defaultPath) => {
    ...
    const result = await dialog.showOpenDialog(window, {
        title: 'Select capture output folder',
        defaultPath: absoluteDefaultPath,
