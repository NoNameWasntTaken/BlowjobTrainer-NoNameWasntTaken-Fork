...
import {
    ...
    preserveVibrationAtom
} from '../atoms/buttplugAtom';
import { store } from '../store';

/**
 * Run a vibration-stopping callback unless a skip-feedback handoff is preserving
 * intensity across a task remount. Use for unmount/idle stops that would otherwise
 * zero vibration during a fast transition. Intentional stops (surface penalty, hold
 * fail/complete, pause) should call stop directly and ignore this guard.
 */
export function stopVibrationUnlessPreserved(stopFn) {
    if (store.get(preserveVibrationAtom)) return;
    stopFn();
}
...
export function useButtplug() {
    ...
    useEffect(() => {
        return () => {
            // Stop vibration when component unmounts, unless a skip-feedback handoff
            // is preserving intensity into the next task.
