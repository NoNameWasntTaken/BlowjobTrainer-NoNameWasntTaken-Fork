...
import { feedbackAtom, sfxAtom, stopVoiceRequestAtom } from '../../atoms/audioAtom';
        ...
        <React.Fragment>
            ...
            {playState === PlayState.PLAYING && (
                <div>
                        ...
                        {currentLevel.currentTask != null && currentLevel.currentTask.type !== TaskType.CALIBRATION && (
                            <CountdownTimer
                                ...
                                onTimeElapsed={() => {
                                    ...
                                    if ((task?.type === TaskType.REST || task?.type === TaskType.REST_BALL) && task?.ballsBonus) {
                                        return
                                    }
                                    // these just transition to next task
                                    if (task?.type === TaskType.REST ||
                                        task?.type === TaskType.GETREADY ||
                                        task?.type === TaskType.REST_BALL) {
                                        gotoNextTask()
                                    }
                    ...
                    {(currentLevel?.currentTask?.type === TaskType.REST || currentLevel?.currentTask?.type === TaskType.REST_BALL) &&
