...
// Combined shaft percentage (derived) - all grids
...
// Combined shaft-only percentage (excludes balls grids) - used for depth/grace
export const combinedShaftOnlyPercentAtom = atom((get) => {
    const grids = get(gridsAtom)
    const ownership = get(gridSquareOwnershipAtom)
    const gridColors = get(gridColorsAtom)
    ...
    grids.forEach(grid => {
        if (grid.balls === true) return // skip balls grids
        grid.squares.forEach(sq => {
            totalSquares++
            const key = `${sq.x},${sq.y}`
            const entry = gridColors[key]
...
// Combined balls percentage (balls grids only)
export const combinedBallsPercentAtom = atom((get) => {
    const grids = get(gridsAtom)
    const ownership = get(gridSquareOwnershipAtom)
    const gridColors = get(gridColorsAtom)
    
    const ballsGrids = grids.filter(g => g.balls === true)
    if (ballsGrids.length === 0) return 0
    
    let totalSquares = 0
    let coveredSquares = 0
    
    ballsGrids.forEach(grid => {
        grid.squares.forEach(sq => {
            totalSquares++
            const key = `${sq.x},${sq.y}`
            const entry = gridColors[key]
    ...
    return Math.round((coveredSquares / totalSquares) * 100)
})

// Balls covered state (writable) - updated by useGridProcessor with buffer hysteresis
export const ballsCoveredStateAtom = atom(false)

// Balls covered (boolean) - low visibility means balls are hidden/inserted (same logic as shaft depth)
// Derived from ballsCoveredStateAtom which uses buffer hysteresis when hasBallsGrids
export const ballsCoveredAtom = atom((get) => {
    const grids = get(gridsAtom)
    const hasBallsGrids = (Array.isArray(grids) ? grids : []).some(g => g.balls === true)
    if (!hasBallsGrids) return false
    return get(ballsCoveredStateAtom)
})
...
export const ballsDepthPercentAtom = atom(50)
