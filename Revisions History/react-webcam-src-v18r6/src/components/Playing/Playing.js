...
import { TaskType, getTaskSummary } from '../Tasks/task';
...
function Playing() {
    ...
    useEffect(() => {
        ...
        if (isExternalMode && isHeadless && 
            ...
            // Apply calibration data if needed (should already be applied if skip-calibration, but ensure it's applied)
            // Calibration should already be applied in App.js if skip-calibration, but double-check
            
            // Auto-start: equivalent to handleBeginPlay()
    ...
    }, [playState, currentLevel, setPlayState, setCurrentLevel])
    ...
    function gotoNextTask() {
        ...
        if (currentLevel.tasks.length === 0) {
            ...
            // note the time played
            setPlayState(PlayState.NOT_PLAYING)
            // Reset playTime for next game
            setNav(NAV.GAMEOVER)
        }
        // get next task
        else {
            let next = currentLevel.tasks[0]
            let remainingTasks = currentLevel.tasks.slice(1)

            // Check for milestone achievements
            let updatedMilestones = { ...currentLevel.milestones }
            if (next.type === TaskType.HOLDPOSITION || next.type === TaskType.HITDEPTH) {
            ...
            // Execute external program if specified (non-blocking)
    ...
    function onTaskOver(taskSummary, isSuccess) {
        ...
        // get a nice string for the task summary
        const taskDesc = getTaskSummary(currentLevel.currentTask)
        ...
        <React.Fragment>
            {playState === PlayState.NOT_PLAYING && (
                <div className='margin-y'>
                    ...
                    <div className="row-centered">
                        <button className="padding-x button button-primary" onClick={() => handleBeginPlay()}>begin</button>
                    </div>
                </div>
            )}
                ...
                <div>
                        ...
                        {currentLevel.currentTask != null && currentLevel.currentTask.type !== TaskType.CALIBRATION && (
                            <CountdownTimer
                                ...
                                onTimeElapsed={() => {
                                    ...
                                    if (currentLevel.currentTask?.type === TaskType.REST ||
                                        currentLevel.currentTask?.type === TaskType.GETREADY ||
                                        currentLevel.currentTask?.type === TaskType.REST_BALL) {
                                        gotoNextTask()
                                    }
                    ...
                    {currentLevel.currentTask?.type === TaskType.CLAP && (
                        <ClapDetector
                            ...
                            timeLimit={currentLevel.currentTask.timeLimit}
