...
import { TaskType, calculateLevelDuration } from '../Tasks/task'
...
import { levelEditorTaskClipboardAtom } from '../../atoms/levelEditorTaskClipboardAtom'
...
import {
    applyEditorAudioFields,
    allocateLevelExportFileName,
    levelExportFileName,
    normalizeImportedLevel,
    parseLevelFileText,
    stringifyLevelFile,
    triggerBrowserDownload,
} from '../../utils/levelFileFormat'
import {
    pickAndReadLevelJsonDirectory,
    pickDirectoryAndWriteLevelFiles,
} from '../../services/levelBulkFileService'
...
const LevelEditor = () => {
    ...
    const [captureSettingsOpen, setCaptureSettingsOpen] = useState(true)
    const [folderNameSets, setFolderNameSets] = useAtom(customLevelFolderNamesAtom)
    const [clipboardTask, setClipboardTask] = useAtom(levelEditorTaskClipboardAtom)
    ...
    /** 'all' = every custom level; otherwise 1-based folder slot. */
    const [loadExistingFolderFilter, setLoadExistingFolderFilter] = useState('all')
    const [bulkBusy, setBulkBusy] = useState(false)
    const importFileInputRef = useRef(null)

    const folderSlotsWithLevels = useMemo(
        () => getCustomSubfolderSlotsWithLevels(existingLevels),
        [existingLevels]
    )

    const levelsForLoadSelect = useMemo(() => {
        if (loadExistingFolderFilter === 'all') return existingLevels
        return existingLevels.filter(
            (l) => normalizeCustomSubfolder(l.customSubfolder) === loadExistingFolderFilter
        )
    }, [existingLevels, loadExistingFolderFilter])

    useEffect(() => {
        if (
            loadExistingFolderFilter !== 'all' &&
            !folderSlotsWithLevels.includes(loadExistingFolderFilter)
        ) {
            setLoadExistingFolderFilter('all')
        }
    }, [folderSlotsWithLevels, loadExistingFolderFilter])
    ...
    const handleCopyTaskAtIndex = (index) => {
        const task = level.tasks[index]
        if (!task) return
        setClipboardTask(structuredClone(task))
    }

    const handlePasteTask = () => {
        if (clipboardTask == null) return
        const pasted = { ...structuredClone(clipboardTask), id: getRandomInt() }
        setLevel((prev) => {
            ...
            return {
                ...prev,
                tasks: [...tasks.slice(0, insertionIndex), pasted, ...tasks.slice(insertionIndex)],
            }
        })
    }

    const handleClearTaskClipboard = () => {
    ...
    const applyImportedLevelToEditor = (normalized, { editing }) => {
        setLevel(normalized)
        setAudioPackId(normalized.audioPackId || null)
        const bg = normalized.backgroundMusicId
        if (bg === 'use_selected') setBackgroundMusicId('use_selected')
        else if (bg && bg !== '') setBackgroundMusicId(bg)
        else setBackgroundMusicId('')
    ...
    const handleExport = () => {
        ...
        const payloadLevel = applyEditorAudioFields(level, audioPackId, backgroundMusicId)
        triggerBrowserDownload(levelExportFileName(level), stringifyLevelFile(payloadLevel))
    }
    ...
    const handleImport = (event) => {
        ...
        reader.onload = (e) => {
            try {
                const parsed = parseLevelFileText(e.target.result)
                if (!parsed.ok) {
                    alert(parsed.error || 'Invalid level file format')
                    return
                }
                const existsImp = (id) => levelManager.getLevel(id) != null
                const normalized = normalizeImportedLevel(parsed.level, existsImp)
                applyImportedLevelToEditor(normalized, { editing: false })
    ...
    const handleExportAll = async () => {
        const levels = levelsForLoadSelect
        if (levels.length === 0) {
            alert('No custom levels to export for the selected folder.')
            return
        }
        setBulkBusy(true)
        try {
            const usedNames = new Set()
            const files = levels.map((stored) => ({
                filename: allocateLevelExportFileName(stored, usedNames),
                contents: stringifyLevelFile(stored),
            }))
            const result = await pickDirectoryAndWriteLevelFiles(files, {
                title: 'Export custom levels',
                zipName: 'custom-levels.zip',
            })
            if (result.canceled) return
            const writeErrors = result.errors || []
            const n = result.written?.length ?? 0
            let message = result.zipDownloaded
                ? `Exported ${n} level${n === 1 ? '' : 's'} as a ZIP download.`
                : `Exported ${n} level${n === 1 ? '' : 's'}${result.directory ? ` to ${result.directory}` : ''}.`
            if (writeErrors.length > 0) {
                message += `\n${writeErrors.length} file${writeErrors.length === 1 ? '' : 's'} failed to write.`
            }
            alert(message)
        } catch (error) {
            alert(`Error exporting levels: ${error.message}`)
        } finally {
            setBulkBusy(false)
        }
    }
    ...
    const handleImportAll = async () => {
        setBulkBusy(true)
        try {
            const picked = await pickAndReadLevelJsonDirectory({ title: 'Import custom levels' })
            if (picked.canceled) return

            const skipped = []
            for (const err of picked.errors || []) {
                skipped.push({ file: err.name, error: err.error })
            }

            const parsed = []
            const seenIds = new Set()
            picked.files.forEach((entry, index) => {
                const result = parseLevelFileText(entry.text)
                if (!result.ok) {
                    skipped.push({ file: entry.name, error: result.error })
                    return
                }
                const fallbackId = `custom-${Date.now()}-${index}`
                const id = result.level.id || fallbackId
                if (levelManager.isDefaultLevel(id)) {
                    skipped.push({ file: entry.name, error: 'Cannot overwrite a default level' })
                    return
                }
                if (seenIds.has(id)) {
                    skipped.push({ file: entry.name, error: `Duplicate level id "${id}" in this folder` })
                    return
                }
                seenIds.add(id)
                parsed.push({ file: entry.name, raw: result.level, id, fallbackId })
            })

            if (parsed.length === 0) {
                const extra = skipped.length > 0
                    ? `\nSkipped ${skipped.length}: ${skipped.map((s) => `${s.file} (${s.error})`).join('; ')}`
                    : ''
                alert(`No valid level files found in that directory.${extra}`)
                return
            }

            const existingCustomIds = new Set(levelManager.getCustomLevels().map((l) => l.id))
            const overwriteCount = parsed.filter((p) => existingCustomIds.has(p.id)).length
            if (overwriteCount > 0) {
                const confirmed = window.confirm(
                    `Importing may overwrite ${overwriteCount} existing custom level${overwriteCount === 1 ? '' : 's'}. Continue?`
                )
                if (!confirmed) return
            }

            const batchIds = new Set(parsed.map((p) => p.id))
            const levelExistsIncludingBatch = (id) =>
                levelManager.getLevel(id) != null || batchIds.has(id)

            const imported = []
            const failed = []
            const notes = []
            const openId = level.id

            for (const item of parsed) {
                try {
                    const normalized = normalizeImportedLevel(
                        item.raw,
                        levelExistsIncludingBatch,
                        item.fallbackId
                    )
                    levelManager.saveLevel(normalized)
                    imported.push(item)
                } catch (error) {
                    failed.push({ file: item.file, error: error.message })
                }
            }
            ...
            for (const item of imported) {
                try {
                    const existsNow = (id) => levelManager.getLevel(id) != null
                    const restored = normalizeImportedLevel(item.raw, existsNow, item.fallbackId)
                    levelManager.saveLevel(restored)
                } catch (error) {
                    notes.push({
                        file: item.file,
                        error: `Saved, but prerequisites could not be restored: ${error.message}`,
                    })
                }
            }

            refreshExistingLevels()
            if (importedIds.has(openId)) {
                handleLoadLevel(openId)
            }

            const overwritten = parsed.filter(
                (p) => importedIds.has(p.id) && existingCustomIds.has(p.id)
            ).length
            const importedCount = importedIds.size
            const parts = [
                `Imported ${importedCount} level${importedCount === 1 ? '' : 's'}`,
            ]
            if (overwritten > 0) parts[0] += ` (${overwritten} overwritten)`
            if (skipped.length > 0) {
                parts.push(`Skipped ${skipped.length} file${skipped.length === 1 ? '' : 's'}`)
            }
            if (failed.length > 0) {
            ...
            const detailLines = [...skipped, ...failed, ...notes]
                .slice(0, 8)
    ...
    return (
        <div className="level-editor">
            <h2 className="tab-title">Level Editor</h2>
                ...
                <div className="margin-y padding-x padding-y-md">
                    ...
                    >
                        <div style={{ width: '100%', minWidth: 0 }}>
                            <p className="margin-y-sm" style={{ marginBottom: '4px' }}>
                                Folder
                            </p>
                            <select
                                className="button"
                                ...
                                value={
                                    loadExistingFolderFilter === 'all'
                                        ? 'all'
                                        : String(loadExistingFolderFilter)
                                }
                                onChange={(e) => {
                                    const v = e.target.value
                                    if (v === 'all') setLoadExistingFolderFilter('all')
                                    else setLoadExistingFolderFilter(Number(v))
                                }}
                            >
                                <option value="all">All Folders</option>
                                {folderSlotsWithLevels.map((slot) => (
                        ...
                        <div style={{ width: '100%', minWidth: 0 }}>
                            ...
                            <select
                                key={`load-level-${loadExistingFolderFilter}`}
                                className="button"
                                style={{
                                    display: 'block',
                                    width: '100%',
            ...
            <div className="row flex-wrap margin-y-sm" >
                ...
                <button
                    type="button"
                    className="button padding-x margin-x"
            ...
            <div className="row flex-wrap margin-y-sm">
                <button
                    ...
                    disabled={bulkBusy || levelsForLoadSelect.length === 0}
                >
                    Export All Levels
                </button>
                <button
            ...
            <div className='grid-two' >
                ...
                <div>
                    <div className='border-grey margin-y padding-x padding-y-md'>
                        ...
                        {/* <div className='margin-y-sm'>
                            ...
                            />
                            {isEditing && <small style={{ color: 'var(--muted)' }}>ID cannot be changed after saving</small>}
                        </div> */}
                        ...
                        <div className='margin-y-sm' style={{ marginTop: '2rem' }}>
                            <button
                                type="button"
                                className="button"
                                style={{ width: '100%', textAlign: 'left' }}
                                onClick={() => setCaptureSettingsOpen((o) => !o)}
                            >
                                Capture Settings {captureSettingsOpen ? '▼' : '▶'}
                            </button>
                            {captureSettingsOpen && (
                                <div
                                    style={{
                                ...
                                >
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                                        <input
                                            type="checkbox"
                                            checked={!!level.allowsCaptures}
                                            onChange={(e) =>
                                                setLevel({ ...level, allowsCaptures: e.target.checked })
                                            }
                                        />
                                        Allow captures for this level
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                                        <input
                                            type="checkbox"
                                            checked={!!level.allowsHiddenCaptures}
                                            onChange={(e) =>
                                                setLevel({ ...level, allowsHiddenCaptures: e.target.checked })
                                            }
                                        />
                                        Allow hidden capture notifications
                                    </label>
                                    {level.allowsCaptures && (
                                        <>
                                            <NumberControl
                                                label="Chance of capture (%)"
                                                centerRow
                                                value={level.chanceOfCapture ?? 25}
                                                setValue={(fn) =>
                                                    setLevel((prev) => ({
                                                        ...prev,
                                                        chanceOfCapture: fn(prev.chanceOfCapture ?? 25),
                                                    }))
                                                }
                                                min={1}
                                                max={100}
                                            ...
                                            />
                                            <NumberControl
                                                label="Compounding chance (%)"
                                                centerRow
                                                value={level.compoundingChance ?? 5}
                                                setValue={(fn) =>
                                                    setLevel((prev) => ({
                                                        ...prev,
                                                        compoundingChance: fn(prev.compoundingChance ?? 5),
                                                    }))
                                                }
                                                min={0}
                                                max={100}
                                            ...
                                            />
                                            <NumberControl
                                                label="Cooldown (seconds, min 5)"
                                                centerRow
                                                value={Math.max(5, level.captureCooldown ?? 10)}
                                                setValue={(fn) =>
                                                    setLevel((prev) => ({
                                                        ...prev,
                                                        captureCooldown: Math.max(5, fn(prev.captureCooldown ?? 10)),
                                                    }))
                                                }
                                                min={5}
                                                max={300}
                                            ...
                                            />
                                            <NumberControl
                                                label="Type bias (0 = photo, 100 = video)"
                                                centerRow
                                                value={level.captureTypeBias ?? 50}
                                                setValue={(fn) =>
                                                    setLevel((prev) => ({
                                                        ...prev,
                                                        captureTypeBias: fn(prev.captureTypeBias ?? 50),
                                                    }))
                                                }
                                                min={0}
                                                max={100}
                                            ...
                                            <NumberControl
                                                label="Photo limit"
                                                centerRow
                                                value={level.photoCaptureLimit ?? 3}
                                                setValue={(fn) =>
                                                    setLevel((prev) => ({
                                                        ...prev,
                                                        photoCaptureLimit: fn(prev.photoCaptureLimit ?? 3),
                                                    }))
                                                }
                                                min={0}
                                                max={999}
                                            ...
                                            <NumberControl
                                                label="Video limit"
                                                centerRow
                                                value={level.videoCaptureLimit ?? 3}
                                                setValue={(fn) =>
                                                    setLevel((prev) => ({
                                                        ...prev,
                                                        videoCaptureLimit: fn(prev.videoCaptureLimit ?? 3),
                                                    }))
                                                }
                                                min={0}
                                                max={999}
                                                step={1}
                                            />
                                            <label className="margin-y-sm" style={{ display: 'block' }}>
                                                Capture output directory (optional)
                                                <div
                                                ...
                                                >
                                                    <input
                                                        ...
                                                        value={level.captureOutput ?? ''}
                                                        placeholder="Leave empty for automatic resolution"
                                                        onChange={(e) =>
                                                            setLevel({ ...level, captureOutput: e.target.value })
                                                        }
                                                    />
                                                    <button
                                                        type="button"
                                                        className="button"
                                                        disabled={!executableService.supportsDirectoryPicker}
                                                        title={
                                                            executableService.supportsDirectoryPicker
                                                                ? ''
                                                                : 'Folder picker only available in Electron (desktop) mode'
                                                        }
                                                        onClick={async () => {
                                                            const hint = level.captureOutput || ''
                                                            const selected = await executableService.pickDirectory(hint)
                                                            if (selected) {
                                                                setLevel((prev) => ({
                                                                    ...prev,
                                            ...
                                            <label style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                <input
                                                    type="checkbox"
                                                    checked={!!level.showStandbyInactiveIcons}
                                                    onChange={(e) =>
                                                        setLevel({
                                                            ...level,
                                                            showStandbyInactiveIcons: e.target.checked,
                                                        })
                                                    }
                                                />
                                                Show standby / inactive icons
                                            </label>
                                            {(Number(level.photoCaptureLimit) || 0) +
                                                (Number(level.videoCaptureLimit) || 0) ===
                                                0 && (
                                                <p style={{ color: 'var(--danger)', fontSize: 13, marginTop: 8 }}>
                                                    At least one capture type should have a limit greater than 0 for
                                                    captures to run.
                                                </p>
                                            )}
                                        </>
                ...
                <div>
                    <div className='border-grey margin-y padding-x padding-y-md'>
                        <h3>Preview</h3>
                        <p><strong>Level ID:</strong> <code style={{ fontFamily: 'monospace', backgroundColor: 'var(--surface-soft)', padding: '2px 6px', borderRadius: '3px' }}>{level.id || 'Not set'}</code></p>
                    ...
                    <div className='border-grey margin-y padding-x padding-y-md'>
                        ...
                        <p
                            className="margin-y-sm"
                            style={{ color: 'var(--muted)', fontSize: '13px', maxWidth: '40rem' }}
                        >
            ...
            <div className="task-builder-section">
                <TaskBuilder
                    ...
                    onCopyTask={handleCopyTaskAtIndex}
                    onPasteTask={handlePasteTask}
                    onClearClipboard={handleClearTaskClipboard}
                />
            </div>
