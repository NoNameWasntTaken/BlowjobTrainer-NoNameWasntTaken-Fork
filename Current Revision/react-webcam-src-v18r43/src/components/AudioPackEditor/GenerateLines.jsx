...
function buildClearedLineState(category, pack) {
    const next = {}
    if (category) {
        for (const key of getGeneratableKeys(category, pack)) {
...
function collectBatchJobs(lineState, category, pack) {
    const jobs = []
    for (const key of getGeneratableKeys(category, pack)) {
...
function collectFailedJobs(lineState, category, pack) {
    const jobs = []
    for (const key of getGeneratableKeys(category, pack)) {
        const eventId = makeEventId(category, key)
        const event = lineState[eventId]
...
function countFailedVariants(lineState, category, pack) {
    let count = 0
    for (const key of getGeneratableKeys(category, pack)) {
...
const GenerateLines = ({ pack, isEditing, onUpdateAudioFiles, onAutosavePack }) => {
    ...
    const [autosaveOnGenerate, setAutosaveOnGenerate] = useAtom(autosaveOnGenerateAtom)
    ...
    const availableKeys = useMemo(
        () => (selectedCategory ? getGeneratableKeys(selectedCategory, pack) : []),
    ...
    useEffect(() => {
        ...
        const keys = getGeneratableKeys(selectedCategory, pack)
        if (keys.length === 0) {
            setSelectedKey('')
            return
        }
    ...
    }, [selectedCategory, selectedKey, pack])
    ...
    const batchCount = useMemo(
        () => collectBatchJobs(lineState, selectedCategory, pack).length,
        [lineState, selectedCategory, pack]
    )
    ...
    const failedCount = useMemo(
        () => countFailedVariants(lineState, selectedCategory, pack),
        [lineState, selectedCategory, pack]
    )
    ...
    const maybeAutosaveAfterRound = useCallback(
        (result) => {
            if (!autosaveOnGenerate || !onAutosavePack) return
            if (result?.cancelled) return
            if (result?.succeeded === 0) return

            const saveResult = onAutosavePack()
            if (saveResult?.ok) {
                setAutosaveStatus('saved')
            } else if (saveResult?.reason === 'missing_name') {
                setAutosaveStatus('missing_name')
            }
        },
        [autosaveOnGenerate, onAutosavePack]
    )
    ...
    const runJobs = useCallback(
        async (jobs) => {
            ...
            await runBatch(jobs, voiceSettings, {
                ...
                onComplete: maybeAutosaveAfterRound,
            })
        },
        [
            canGenerate,
    ...
    const handleGenerateVariant = async (category, key, variantId) => {
        ...
        try {
            ...
            maybeAutosaveAfterRound({ succeeded: 1, failed: 0, cancelled: false })
        } catch (err) {
            updateVariant(eventId, variantId, {
                status: STATUS.ERROR,
                error: err.message || 'Generation failed',
    ...
    const handleGenerateBatch = () => {
        const jobs = collectBatchJobs(lineState, selectedCategory, pack)
        runJobs(jobs)
    }

    const handleRetryFailed = () => {
        const jobs = collectFailedJobs(lineState, selectedCategory, pack)
        runJobs(jobs)
    }
    ...
    const handleClearAllLines = useCallback(() => {
        ...
        setSingleGeneratingId(null)
        setLineState(buildClearedLineState(selectedCategory, pack))
    }, [hasLineContent, isRunning, cancelBatch, selectedCategory, pack])
    ...
    return (
        <div className="generate-lines">
            ...
            <div className="generate-lines-category-picker">
                ...
                {selectedCategory === 'Custom' && availableKeys.length === 0 && (
                    <p className="generate-lines-hint">
                        This pack has no custom voice lines yet. Add a custom voice line in the Categories tab, then generate it here.
                    </p>
                )}
            </div>
            ...
            <div className="generate-lines-batch-toolbar">
                ...
                <label className="generate-lines-autosave">
                    <input
                        type="checkbox"
                        checked={autosaveOnGenerate}
                        onChange={(e) => {
                            setAutosaveOnGenerate(e.target.checked)
                            setAutosaveStatus(null)
                        }}
                        disabled={!isEditing}
                    />
                ...
                {autosaveStatus === 'saved' && !isRunning && (
                    <span className="generate-lines-summary generate-lines-autosave-status">
                        Pack saved
                    </span>
                )}
                {autosaveOnGenerate && autosaveStatus === 'missing_name' && !isRunning && (
                    <span className="generate-lines-summary generate-lines-autosave-warning">
                        Pack name required for autosave
                    </span>
                )}
            </div>
            {autosaveOnGenerate && !pack.name?.trim() && isEditing && (
                <p className="generate-lines-hint generate-lines-autosave-hint">
                    Enter a pack name in metadata to enable autosave.
                </p>
            )}
        </div>
    )
