...
import { combinedBallsPercentAtom, ballsDepthPercentAtom, gridsAtom } from '../../atoms/gridAtoms';
...
function EndlessDive({ task, onTaskOver, priorScore = 0 }) {
    ...
    const ballsCovered = hasBallsGrids && combinedBallsPercent < ballsDepthPercent;
    useEffect(() => {
    ...
    useEffect(() => {
        if (motion.dir === 'up') {
            ...
            if (motion.holdTime >= HOLD_THRESHOLD) {
                ...
                setTotals(newTotals);
                totalsRef.current = newTotals;
                const depthFactor = DEPTH_GRACE_FACTOR[depth] ?? 1;
