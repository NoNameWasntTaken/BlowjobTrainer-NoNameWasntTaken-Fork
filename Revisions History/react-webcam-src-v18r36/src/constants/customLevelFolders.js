...
/**
 * Sorted 1-based subfolder slots that contain at least one level in `levels`,
 * capped at {@link CUSTOM_SUBFOLDER_COUNT} entries.
 *
 * @param {Array<{ customSubfolder?: unknown }>} levels
 * @returns {number[]}
 */
export function getCustomSubfolderSlotsWithLevels(levels) {
    if (!Array.isArray(levels)) return []
    const set = new Set()
    for (const l of levels) {
        set.add(normalizeCustomSubfolder(l?.customSubfolder))
    }
    return [...set].sort((a, b) => a - b).slice(0, CUSTOM_SUBFOLDER_COUNT)
}
