 ...
 * Does not call reset — the onAfterDecodeRef callback must perform all resets.
 *
 ...
 * @param {React.MutableRefObject<boolean>} opts.pcmFeedAllowedRef When false, PCM is not fed to Sherpa.
 ...
 * @param {React.MutableRefObject<((args: { text: string, isEndpoint: boolean, reset: () => void }) => void) | null | undefined>} opts.onAfterDecodeRef
...
export function useSherpaMicTap({
    ...
    useEffect(() => {
        if (!enabled) {
            ...
            releaseOnlineStream(sessionRef.current)
        ...
        ;(async () => {
            try {
                ...
                const session = await acquireOnlineStream()
                if (cancelled) {
                    session.release()
                    return
                }
                sessionRef.current = session
                session.setResultHandler(({ text, isEndpoint }) => {
                    ...
                    if (onAfter) {
                        onAfter({ text, isEndpoint, reset: () => session.reset() })
                    }
                })
                ...
                setSpeakPcmChunkHandler(({ pcm, rms }) => {
                    ...
                    if (!pcmFeedAllowedRef.current) return
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps -- pcmFeedAllowedRef, completedRef, callback refs intentionally omitted
    }, [enabled, resetKey])
