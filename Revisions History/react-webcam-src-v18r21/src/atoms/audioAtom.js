...
/** Lifecycle of `playVoice(currentTask.audio)` only — not feedback / Task.GOOD lines. */
export const taskInstructionVoicePhaseAtom = atom('idle')

/** Speak task: whether mic recognition may advance scoring (Playing derives from phase + audio). */
...
/**
 * `MediaDeviceInfo.deviceId` for `audioinput`. Legacy stored `null` (omit deviceId) is normalized on Audio tab
 * to the default-labeled device id when `MicInputDevicePicker` loads the device list.
 */
