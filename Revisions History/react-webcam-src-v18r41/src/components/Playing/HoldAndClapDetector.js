...
// target depth this long before the hold officially starts, so a leftover stroke from the
// previous task doesn't auto-start the hold or trigger a surface penalty.
...
function HoldAndClapDetector({ task, onTaskComplete, onSignalCaptureWindow, isLastTask }) {
    ...
    // Skip-feedback handoff: while armed, the hold stays parked until a brief stable dwell
    // at the target depth. dwellTimerRef holds the pending dwell timer.
    ...
    useEffect(() => {
        ...
        // Arm a dwell timer only once stably at the target depth; cancel it if the player
        // leaves before it fires. When it fires, the hold starts normally.
