...
import {
    normalizeCustomLevelFolderNamesArray,
...
const STORAGE_KEYS = {
    ...
    BACKGROUND_MUSIC_TRACKS: 'bg_music_tracks',
    ACTIVE_BACKGROUND_MUSIC: 'active_bg_music_id',
    CUSTOM_LEVEL_FOLDER_NAMES: 'custom_level_folder_names',
}

const PLAYER_PROFILES_STORAGE_KEY = 'playerProfiles'
...
export const storageService = {
    ...
    /**
     * Global custom level subfolder display names (Training + Level Editor).
     * Two rows: concealed (hidden content off) vs revealed (hidden content on).
     */
    getCustomLevelFolderNameSets: () => {
        try {
            const raw = localStorage.getItem(STORAGE_KEYS.CUSTOM_LEVEL_FOLDER_NAMES)
            if (raw != null) {
                const parsed = JSON.parse(raw)
                return normalizeCustomLevelFolderNameSets(parsed)
            }
        } catch {
            /* fall through to migrate / default */
        }

        const fromProfiles = storageService._migrateCustomFolderNamesFromPlayerProfiles()
        if (fromProfiles) {
            storageService.saveCustomLevelFolderNameSets(fromProfiles)
            return fromProfiles
        }

        const defs = createDefaultCustomLevelFolderNameSets()
        storageService.saveCustomLevelFolderNameSets(defs)
        return defs
    },

    saveCustomLevelFolderNameSets: (sets) => {
        const normalized = normalizeCustomLevelFolderNameSets(sets)
        localStorage.setItem(
            STORAGE_KEYS.CUSTOM_LEVEL_FOLDER_NAMES,
            JSON.stringify(normalized)
        )
        return normalized
    },

    /** @private One-time seed from legacy per-profile customLevelFolderNames */
    _migrateCustomFolderNamesFromPlayerProfiles: () => {
        try {
            const raw = localStorage.getItem(PLAYER_PROFILES_STORAGE_KEY)
            if (raw == null) return null
            const parsed = JSON.parse(raw)
            if (!parsed?.profiles || typeof parsed.profiles !== 'object') return null
            const activeId = parsed.activeProfileId || 'default'
            const prof =
                parsed.profiles[activeId] ||
                parsed.profiles[Object.keys(parsed.profiles)[0]]
            const names = prof?.customLevelFolderNames
            if (!Array.isArray(names)) return null
            const row = normalizeCustomLevelFolderNamesArray(names)
            return { concealed: [...row], revealed: [...row] }
        } catch {
            return null
        }
    },
    ...
    // Background music tracks (metadata only; blobs in IndexedDB via audioFileService, packId 'bg-music')
    getAllBackgroundTracks: () => {
        const data = localStorage.getItem(STORAGE_KEYS.BACKGROUND_MUSIC_TRACKS)
        return data ? JSON.parse(data) : {}
    },

    saveBackgroundTrack: (meta) => {
        const tracks = storageService.getAllBackgroundTracks()
        tracks[meta.id] = meta
        localStorage.setItem(STORAGE_KEYS.BACKGROUND_MUSIC_TRACKS, JSON.stringify(tracks))
        return meta
    },

    deleteBackgroundTrackMeta: (trackId) => {
        const tracks = storageService.getAllBackgroundTracks()
    ...
    /**
     * Legacy helpers for the same key as activeBackgroundTrackIdAtom (atomWithStorage).
     * Do not call alongside store.set(activeBackgroundTrackIdAtom) — use musicTrackManager.setActiveTrack / the atom only.
     */
    setActiveBackgroundMusic: (trackId) => {
        if (trackId) {
            localStorage.setItem(STORAGE_KEYS.ACTIVE_BACKGROUND_MUSIC, trackId)
        } else {
            localStorage.removeItem(STORAGE_KEYS.ACTIVE_BACKGROUND_MUSIC)
        }
    },

    getActiveBackgroundMusic: () => {
        return localStorage.getItem(STORAGE_KEYS.ACTIVE_BACKGROUND_MUSIC) || null
    },
    ...
    /**
     * Remove custom voice packs, levels, background track metadata, and active ids.
     * Does not remove `playerProfiles` — reset profiles via Jotai separately.
     */
    clearAllCustomContent: () => {
        try {
            localStorage.removeItem(STORAGE_KEYS.AUDIO_PACKS);
            localStorage.removeItem(STORAGE_KEYS.LEVELS);
            localStorage.removeItem(STORAGE_KEYS.BACKGROUND_MUSIC_TRACKS);
            localStorage.removeItem(STORAGE_KEYS.ACTIVE_AUDIO_PACK);
            localStorage.removeItem(STORAGE_KEYS.ACTIVE_BACKGROUND_MUSIC);
        } catch (e) {
            console.warn('clearAllCustomContent failed', e);
        }
    }
}
...
export const audioFileService = {
    ...
    // Delete IndexedDB blobs for one background-music track (packId bg-music, filePath prefix trackId/)
    deleteBackgroundMusicTrackFiles: async (trackId) => {
        const database = await initDB()
        const transaction = database.transaction([STORE_AUDIO_FILES], 'readwrite')
        const store = transaction.objectStore(STORE_AUDIO_FILES)
        const prefix = `bg-music:${trackId}/`
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => {
                ...
                if (cursor) {
                    ...
                    audioFileService.revokePackUrls('bg-music')
                    resolve()
                }
            }
            request.onerror = () => reject(request.error)
    ...
    /** Delete every blob in the audioFiles object store (custom packs + bg music files). */
    clearAllAudioFiles: async () => {
        const database = await initDB()
        const transaction = database.transaction([STORE_AUDIO_FILES], 'readwrite')
        const idbStore = transaction.objectStore(STORE_AUDIO_FILES)
        return new Promise((resolve, reject) => {
            const request = idbStore.openCursor()
            request.onsuccess = (event) => {
                const cursor = event.target.result
                if (cursor) {
                    cursor.delete()
