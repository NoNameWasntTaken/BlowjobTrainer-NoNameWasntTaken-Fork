...
import { clapSensitivityAtom, clapTestEnabledAtom } from '../../atoms/markersAtoms'
...
function ClapDetector({ targetClaps, onTaskComplete, timeLimit, isCalibration = false }) {
    ...
    const [clapTestEnabled, setClapTestEnabled] = useAtom(clapTestEnabledAtom)
    ...
    }, [targetClaps, onTaskComplete, setFeedback, isCalibration, lastFeedbackClap, nextFeedbackIn])

    const micEnabledForClap = !isCalibration || clapTestEnabled
    ...
    return (
        <div className="clap-detector">
            ...
            {isCalibration && <p className="help margin-y-sm"><b>Adjust the threshold </b>value below, then slap yourself (or just clap your hands).  Keep an eye on the <b>counter</b> to see what seems correct. </p>}
            ...
            {isCalibration && <>
                <div className="row-centered">
                    <pre>frequency reading: {micEnabledForClap ? (freqData.high * 100).toFixed(0) : '—'}</pre>
                </div>
                ...
                <div className="row-centered margin-y-sm">
                    <button
                        type="button"
                        className={`button padding-x${clapTestEnabled ? ' button-primary' : ''}`}
                        onClick={() => setClapTestEnabled((v) => !v)}
                        aria-pressed={clapTestEnabled}
                    >
                        {clapTestEnabled ? 'Clap Test On' : 'Clap Test Off'}
                    </button>
                </div>
                <div className="row-centered">
