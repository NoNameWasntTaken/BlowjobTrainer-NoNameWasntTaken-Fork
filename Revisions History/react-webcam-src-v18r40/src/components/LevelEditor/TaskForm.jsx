...
import {
    ...
    getHoldProgressHalfAudioOptions,
    getHoldProgress3QAudioOptions,
 ...
 */
const TaskForm = ({ task, taskNumber, onChange, onDelete, audioPackId = null }) => {
    ...
    /** Collapsed "Capture options" for applicable task types */
    ...
    }, [task.id, task.type, task.targetDepth])

    /** Voice-Halfway / Voice-3Q use the same select width as Voice-Main (from main hold option labels). */
    const holdVoiceLineSelectStyle = React.useMemo(() => {
        if (task.type !== TaskType.HOLDPOSITION) return undefined
        const opts = getAudioOptionsForTaskType(
            task.type,
        ...
        let maxLen = 4 // "None"
        for (const o of opts) {
            const s = String(o.label ?? o.value ?? '')
            if (s.length > maxLen) maxLen = s.length
        }
        const ch = Math.min(Math.max(maxLen + 2, 20), 56)
        return {
            width: `${ch}ch`,
            minWidth: `${ch}ch`,
            boxSizing: 'border-box',
    ...
    }, [task.type, task.showCustomVoiceLines, audioPackId, pack?.customNames])
    ...
    const updateTask = (updates) => {
        ...
        if (updatedTask.type === TaskType.HOLDPOSITION) {
            ...
            // Halfway / 3Q cues track target depth (same as createHOLDTask)
            if (hasDepthChange || taskTypeChanged) {
                const d = updatedTask.targetDepth || 1
                updatedTask.audioHalfway = getDefaultHoldAudioHalfway(d)
                updatedTask.audioThreeQuarter = getDefaultHoldAudioThreeQuarter(d)
            }
        }
        ...
        if (updatedTask.type === TaskType.HITDEPTH) {
            ...
            // Auto-calculate timeLimit when repeat changes
    ...
    const executableField = (
        <div style={{ width: '100%', marginTop: '8px' }}>
            ...
            <small style={{ color: '#666', display: 'block', marginTop: '4px', textAlign: 'center' }}>
                {!isElectron
                    ? 'Script execution only available in packaged Electron app. Script path entry allowed but not functional.'
                    : 'Script runs automatically when this task starts during gameplay. Manual path entry allowed, but using file picker is recommended.'}
            </small>
        </div>
    );

    const captureOptionsSection = captureSupport.any ? (
        <div
            style={{
                marginTop: 8,
                marginBottom: 8,
            ...
            {captureOpen && (
                <>
                    ...
                    <small style={{ color: '#666', display: 'block', marginTop: 4 }}>
                        Higher = more likely visible notification + sound. Use &lt; 100 for possible
                        hidden captures when the profile and level allow it.
                    </small>
                </>
            )}
        </div>
                                ...
                                <div key={idx} className="row flex-wrap" style={{ gap: '8px', alignItems: 'center', marginBottom: '8px', padding: '8px', backgroundColor: '#eee', borderRadius: '4px' }}>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                        <button
                                            ...
                                            disabled={idx === 0}
                                            style={{ padding: '2px 6px', fontSize: '12px', opacity: idx === 0 ? 0.3 : 1 }}
                                        >
                                            ↑
                                        </button>
                                        <button
                                            ...
                                            style={{ padding: '2px 6px', fontSize: '12px', opacity: idx === (task.scoreEvents || []).length - 1 ? 0.3 : 1 }}
                                        >
                                            ↓
                                        </button>
                                    </div>
                                        ...
                                        <div style={ENDLESS_SCORE_EVENT_PLAY_SOUND_ROW_STYLE}>
                                            ...
                                            >
                                                ...
                                                {(soundUsesCustomVoiceLines
                                                    ? [...EVENT_SOUND_OPTIONS, ...getCustomAudioOptions(audioPackId, pack?.customNames ?? {})]
                                                    : EVENT_SOUND_OPTIONS
                                                ).map((o) => (
                                                ...
                                                <div style={ENDLESS_SCORE_EVENT_OPTIONAL_VOICE_INNER_ROW_STYLE}>
                                                    ...
                                                    >
                                                        ...
                                                        {(vibrationUsesCustomVoiceLines
                                                            ? [
                                                                  ...EVENT_SOUND_OPTIONS,
                                                                  ...getCustomAudioOptions(audioPackId, pack?.customNames ?? {}),
                                                              ]
                                                            : EVENT_SOUND_OPTIONS
                                                        ).map((o) => (
        ...
        <div className="border-grey padding-x-sm padding-y-sm margin-y" style={{ backgroundColor: '#f9f9f9' }}>
            <div className="row-space-between">
                <div className="row flex-wrap" style={{ gap: '8px', alignItems: 'center' }}>
                    {taskNumber != null && (
                        <span style={{ fontWeight: 'bold', color: '#555' }}>#{taskNumber}</span>
                    )}
                    <div>
                        <select
                    ...
                    {task.type === TaskType.HOLDPOSITION ? (
                        <>
                            ...
                            <AudioSelector
                                ...
                                onChange={(audio) => {
                                    if (holdVoiceSlot === HOLD_VOICE_MAIN) {
                                        updateTask({ audio })
                                    } else if (holdVoiceSlot === HOLD_VOICE_HALF) {
                                        updateTask({ audioHalfway: audio || undefined })
                                    } else {
                                        updateTask({ audioThreeQuarter: audio || undefined })
                                    }
                                }}
                                allowedOptions={
                                    holdVoiceSlot === HOLD_VOICE_MAIN
                                ...
                                selectStyle={holdVoiceLineSelectStyle}
                            />
                        </>
                    ) : (
                        <AudioSelector
                            ...
                            allowedOptions={getAudioOptionsForTaskType(task.type, task.showCustomVoiceLines, audioPackId, pack?.customNames ?? {})}
                            audioPackId={audioPackId}
                        />
                    )}
                    <label style={{ display: 'flex', alignItems: 'center', gap: '4px', marginLeft: '2rem', marginTop: '-4px', marginBottom: '2px', paddingBlock: 0, lineHeight: 0.9 }}>
                        <input
                            type="checkbox"
                            checked={!!task.showCustomVoiceLines}
                            onChange={(e) => updateTask({ showCustomVoiceLines: e.target.checked })}
