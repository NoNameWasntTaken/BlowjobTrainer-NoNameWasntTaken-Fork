...
import { canonicalAudioReference, hasAudio } from './customAudio'
...
export const resolveAudioReference = (audioRef, customAudioPack = null, fallbackPack = null) => {
    ...
    if (Array.isArray(audioRef) && audioRef.length > 0 && typeof audioRef[0] === 'string' && audioRef[0].includes('/')) {
        ...
        if (defaultKey) {
            ...
            // Endless.ENDLESS falls back to Level.BEGINT_1_START when pack has no Endless.ENDLESS
            if (defaultKey.category === 'Endless' && defaultKey.key === 'ENDLESS') {
                return resolveAudioReference('Level.BEGINT_1_START', customAudioPack, fallbackPack)
            }
        }
    ...
    // Split on the first dot so voice keys may themselves contain dots.
    if (typeof audioRef === 'string' && audioRef.includes('.')) {
        const canonical = canonicalAudioReference(audioRef)

        const readPack = (pack) => {
            if (!pack?.audioFiles) return undefined
            const normalized = normalizeAudioRef(canonical, pack.audioRefAliases)
            const separator = normalized.indexOf('.')
            if (separator === -1) return undefined
            const category = normalized.slice(0, separator)
            const key = normalized.slice(separator + 1)
            const actualCategory = findCategoryCaseInsensitive(category, pack.audioFiles)
            if (!actualCategory) return undefined
            const value = pack.audioFiles[actualCategory]?.[key]
            if (value === undefined) return undefined
            if (actualCategory.toLowerCase() === 'custom' && !hasAudio(value)) return null
            return value
        }

        const customValue = readPack(customAudioPack)
        if (customValue !== undefined) return customValue

        const fallbackValue = readPack(fallbackPack)
        if (fallbackValue !== undefined) return fallbackValue

        const normalized = normalizeAudioRef(canonical)
        const separator = normalized.indexOf('.')
        ...
        console.warn(`Audio reference not found: ${normalized}`)
        return null
    }
 ...
 * @param {string|string[]} audioPath - Audio file path, array of paths, or Category.KEY string
...
export const getAudioKey = (audioPath, customAudioPack = null) => {
    // If already a Category.KEY string, return as-is
    if (typeof audioPath === 'string' && audioPath.includes('.') && !audioPath.includes('/')) {
        return audioPath
    }
