...
export const useAchievements = () => {
    const [achievementsStored, setAchievementsStored] = useAtom(achievementsStoredAtom)
    ...
    const isUnlocked = (id) => {
        return achievementsStored.unlockedAchievements.includes(id)
    }
    ...
    const getProgress = (id) => {
        ...
        let progress = achievementsStored.achievementProgress[id]
        if (!progress) {
    ...
    const updateProgress = (id, newProgress) => {
        // already unlocked
        if (achievementsStored.unlockedAchievements.includes(id)) return
    ...
    const unlockAchievement = (id) => {
        ...
        console.log('unlockAchievement', id);
    ...
    const checkDeep100 = (summaries) => {
        // Get successful depth 4 actions from hitdepth, updown, and hold tasks
        const depth4Successes = summaries
            ...
            .reduce((total, summary) => {
                ...
                // For hold tasks, use successfulHolds count
    ...
    const checkClapAchievement = (summaries) => {
        ...
        // Get total claps from all CLAP type tasks
        const totalClaps = summaries
            .filter(summary => summary.type === TaskType.CLAP)
