...
import { SPEAK_INSTRUCTION_PCM_OVERLAP_MS } from '../constants/speakAudio'
...
/**
 * Packaged Electron uses `app://`; local file opens use `file:`.
 * Voice lines via `new Audio(url)` match the Audio Pack Editor preview path and avoid fetch+decode on those protocols.
 */
function preferVoiceHtmlAudioFirst() {
...
function AudioPlayer() {
    ...
    const [captureSfx, setCaptureSfx] = useAtom(captureSfxAtom)
    ...
    const setSpeakPcmFeedAllowed = useSetAtom(speakPcmFeedAllowedAtom)
    ...
    const captureSfxAudioRef = useRef(null)
    ...
    const playSfxOnChannel = async (soundFile, channelRef) => {
        if (!soundFile) return
        ...
        let audioUrl
        try {
            audioUrl = await audioManager.getAudioFile(soundFile)
        } catch (e) {
            console.error(`getAudioFile failed for SFX: ${soundFile}`, e)
            return
        }
        if (!audioUrl) {
        ...
        channelRef.current = audio
        audio.onerror = () => console.error(`Error loading SFX file: ${audioUrl}`)
        audio.onended = () => {
            if (channelRef.current === audio) channelRef.current = null
        }
        audio.play().catch((error) => console.error('Error playing SFX:', error))
    }

    const playVoice = (soundFile, { onEnded, signal, resolvedUrl, instructionOverlap } = {}) => {
        ...
        /** Never rejects — avoids unhandled promise rejections from callers / IndexedDB / fetch. */
        const runWithUrl = async (audioUrl) => {
            let pcmOverlapTimerId = null
            const clearPcmOverlapTimer = () => {
                if (pcmOverlapTimerId != null) {
                    clearTimeout(pcmOverlapTimerId)
                    pcmOverlapTimerId = null
                }
            }
            const scheduleInstructionPcmOverlap = (durationSec) => {
                clearPcmOverlapTimer()
                if (!instructionOverlap || signal?.aborted) return
                if (!Number.isFinite(durationSec) || durationSec <= 0) return
                const delay = Math.max(0, durationSec * 1000 - instructionOverlap.overlapMs)
                pcmOverlapTimerId = setTimeout(() => {
                    pcmOverlapTimerId = null
                    if (!signal?.aborted) instructionOverlap.onOpenPcmFeed()
                }, delay)
            }
            if (signal) {
            ...
            try {
                ...
                const preferHtml = preferVoiceHtmlAudioFirst()

                /**
                 * @param {boolean} rejectOnFailure - When true (HTML-first attempt), reject so Web Audio can run next.
                 */
                const playVoiceViaHtmlElement = async (rejectOnFailure) => {
                    if (signal?.aborted) return
                    const audio = new Audio(audioUrl)
                    audio.volume = voiceVolume
                    voiceAudioRef.current = audio

                    if (instructionOverlap) {
                        const tryScheduleOverlap = () => {
                            if (Number.isFinite(audio.duration) && audio.duration > 0) {
                                scheduleInstructionPcmOverlap(audio.duration)
                            }
                        }
                        audio.addEventListener('loadedmetadata', tryScheduleOverlap, { once: true })
                        tryScheduleOverlap()
                    }

                    await new Promise((resolve, reject) => {
                        const clearRefIfOurs = () => {
                        ...
                        /** Load/play failed: reject (HTML-first) or resolve without onEnded (final fallback). */
                        const handleHtmlFailure = (err) => {
                            clearPcmOverlapTimer()
                            clearRefIfOurs()
                            if (rejectOnFailure) {
                                reject(err)
                        ...
                        audio.onerror = () => {
                            console.error(`Error loading voice file: ${audioUrl}`)
                            handleHtmlFailure(new Error('voice media load failed'))
                        }
                ...
                if (preferHtml) {
                    try {
                        await playVoiceViaHtmlElement(true)
                        return
                    } catch (e) {
                        console.warn(
                            'Voice HTMLAudioElement (preferred for file:/app:) failed, trying Web Audio:',
                            e
                        )
                    }
                }
                ...
                try {
                    ...
                    if (instructionOverlap) {
                        scheduleInstructionPcmOverlap(audioBuffer.duration)
                    }
                ...
                try {
                    await playVoiceViaHtmlElement(false)
                } catch (e) {
                    console.error('Voice HTMLAudioElement path failed:', e)
                }
            } catch (e) {
                console.error('playVoice runWithUrl failed:', e)
            }
        }
        ...
        return audioManager.getAudioFile(soundFile).catch((err) => {
            console.error('playVoice getAudioFile failed:', soundFile, err)
            return null
        }).then((audioUrl) => runWithUrl(audioUrl))
    }

    useEffect(() => {
        if (sfxAudioRef.current) sfxAudioRef.current.volume = sfxVolume
        if (captureSfxAudioRef.current) captureSfxAudioRef.current.volume = sfxVolume
    }, [sfxVolume])
    ...
    useEffect(() => {
        ...
        const wasInstructionPlaying = instructionPlayingRef.current
        instructionPlayingRef.current = false
        setFeedback(0)

        const task = currentLevel?.currentTask
        const hasInstructionAudio = task && String(task.audio ?? '').trim()
        if (
            wasInstructionPlaying &&
            hasInstructionAudio &&
            (task.type === TaskType.SPEAK ||
                task.type === TaskType.CLAP ||
                task.type === TaskType.HOLDANDCLAP)
        ) {
            if (task.type === TaskType.SPEAK) {
                setSpeakPcmFeedAllowed(true)
            }
    ...
    useEffect(() => {
        ...
        const playAudio = async () => {
            try {
                if (!cancelled) {
                    await playSfxOnChannel(sfx, sfxAudioRef)
                }
            } catch (e) {
    ...
    useEffect(() => {
        ...
        const playAudio = async () => {
            try {
                if (!cancelled) {
                    await playSfxOnChannel(captureSfx, captureSfxAudioRef)
                }
            } catch (e) {
                console.error('Capture SFX playback error:', e)
            } finally {
                if (!cancelled) {
                    setCaptureSfx(0)
                }
    ...
    useEffect(() => {
        ...
        const instructionSignal = ac.signal
        if (instructionTailTimerRef.current) {
            clearTimeout(instructionTailTimerRef.current)
            instructionTailTimerRef.current = null
        }
        ...
        const instructionVoiceDrivesPhase =
            isSpeak || task.type === TaskType.CLAP || task.type === TaskType.HOLDANDCLAP
        if (instructionVoiceDrivesPhase) {
        ...
        const playAudio = async () => {
            try {
                ...
                await playVoice(task.audio, {
                    ...
                    instructionOverlap:
                        isSpeak && String(task.audio ?? '').trim()
                            ? {
                                  overlapMs: SPEAK_INSTRUCTION_PCM_OVERLAP_MS,
                                  onOpenPcmFeed: () => {
                                      if (!cancelled) setSpeakPcmFeedAllowed(true)
                                  },
                              }
                            : undefined,
                    onEnded: () => {
                        if (!cancelled) setSpeakPcmFeedAllowed(true)
                        instructionPlayingRef.current = false
                        if (cancelled || !instructionVoiceDrivesPhase) return
                        setTaskInstructionVoicePhase('cooldown')
                        if (instructionTailTimerRef.current) {
                            clearTimeout(instructionTailTimerRef.current)
                        }
                ...
                console.error('Instruction voice playback error:', e)
                instructionPlayingRef.current = false
                if (!cancelled && instructionVoiceDrivesPhase) {
                    setSpeakPcmFeedAllowed(true)
                    setTaskInstructionVoicePhase('ready')
                }
            }
    ...
    useEffect(() => {
        ...
        const playAudio = async () => {
            try {
                ...
                console.error('Feedback voice playback error:', e)
            } finally {
                if (!signal.aborted) {
                    setFeedback(0)
                }
    ...
    useEffect(() => {
        return () => {
            ...
            if (captureSfxAudioRef.current) {
                captureSfxAudioRef.current.pause()
                captureSfxAudioRef.current = null
            }
            disconnectVoiceWebAudio()
            if (voiceAudioRef.current) {
                voiceAudioRef.current.pause()
