...
Note that many new features were created with the assumption that the code would be packaged into a standalone application via Electron Forge, rather than hosted on a web browser. To build the application in this way, I opened the project root in a terminal and ran the following commands:
