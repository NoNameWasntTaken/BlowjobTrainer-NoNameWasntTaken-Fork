...
function HoldDepth({ task, onTaskOver }) {
    ...
    }, [remaining, setInstruction, timeTracking.rawTime, timeTracking.effectiveTime, task, holdScores, onTaskOver, stopVibration])

    const holdFailed = useCallback(() => {
        setSfx(AUDIO.Sfx.QUIET)
    ...
    // Calculate score for the hold - placeholder for now
    const calculateScore = useCallback((rawTime, effectiveTime) => {
        ...
        const timeAccuracy = timeDiff / task.time

        let bonusMultiplier = 0
        let grade = Grade.PASS

        // Perfect hold - within max(1s, 10%) of target time
        ...
        // Good hold - within max(3s, 25%) of target time
    ...
    useEffect(() => {
        ...
        if (currentDepth === 0 && targetDepth > 1 && !hasPenalized.current && hasStarted) {
            holdPenalty(SURFACE_PENALTY, targetDepth > 2
                ? AUDIO.Feedback.SURFACE_PENALTY_HARD
                : AUDIO.Feedback.SURFACE_PENALTY_SOFT)
    ...
    }, [diveState.current, task, holdState, holdFailed, holdStart, resetHoldState, setSfx, hasStarted])
    ...
    }, [timeTracking.effectiveTime, task?.time, holdState, holdSuccess])
    ...
    }, [timeTracking.effectiveTime, holdState, task?.time, task?.targetDepth, vibrateSpeed, stopVibration])
