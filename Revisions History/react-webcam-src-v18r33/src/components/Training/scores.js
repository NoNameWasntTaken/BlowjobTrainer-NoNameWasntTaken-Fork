...
export const POINTS_PER_SPEAK_UNIT = 2

/**
 * Perfect score potential for Speak (short: 1 segment; long: split segments × repeat).
 */
export function calcPerfectSpeakScore(task) {
    if (!task?.repeat) return 0
    const segmentCount =
        task.speakMode === SpeakMode.LONG ? Math.max(1, splitSpeakSegments(task.phrase || '').length) : 1
    return POINTS_PER_SPEAK_UNIT * task.repeat * segmentCount
}

export function calcPassSpeakScore(task) {
    return Math.floor(calcPerfectSpeakScore(task) * 0.5)
}
...
const HOLDANDCLAP_DEPTH_FACTOR = { 1: 1.0, 2: 1.25, 3: 1.5, 4: 2.0 }

/**
 * Calculate perfect score for a HoldAndClap task
 */
export function calcPerfectHoldAndClapScore(task) {
    if (!task?.claps || !task?.repeat) return 0
    const depthFactor = HOLDANDCLAP_DEPTH_FACTOR[task.targetDepth] ?? 1.0
    return POINTS_PER_CLAP * task.claps * task.repeat * depthFactor
}

/**
 * Calculate pass score for a HoldAndClap task
 */
export function calcPassHoldAndClapScore(task) {
    return Math.floor(calcPerfectHoldAndClapScore(task) * 0.5)
}

/**
 * Calculate perfect score for an endless task
 */
export function calcPerfectEndlessScore(task) {
    return Math.floor((task.timeLimit || 999) * 1.1)
}

/**
 * Calculate pass score for an endless task
 */
export function calcPassEndlessScore(task) {
    return Math.floor((task.timeLimit || 999) * 0.6)
}
