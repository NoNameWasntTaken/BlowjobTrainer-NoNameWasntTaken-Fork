...
# Revision 26 - Color Sampling Hotfix

Fixed an issue where color sampling used the mirrored x-coordinate, which caused color detection in the previous revision to break.
...
public/electron.js
- config.backgroundMusic; --background-music <trackId> parsing.
...
# Revision 10 - Depth Recognition, Audio Pack, and HoldAndClap Feedback Fixes
...
# Revision 6 - Webcam Support for macOS Builds

Added main-process permission handling so the packaged app can access the camera and microphone when built on macOS.
...
# Revision 3 - Default Levels Custom Audio Fix

Fixed issue where default levels were not using custom audio packs when selected in the Content Library.
...
# Revision 2 - Executable Integration Feature

Implemented optional executable script execution for all task types, allowing external programs to run when tasks start during gameplay. This function is only available when running the packaged desktop app - not supported in browser.
...
*NOTE: This behavior was defined, but not functional due to a bug. This is corrected in Revision 3.
