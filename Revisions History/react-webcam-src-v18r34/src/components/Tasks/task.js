...
} from "../LevelEditor/taskAudioConfig";
...
/** Auto-calculated time limit for HITDEPTH: 10 + repeat * 7 */
export const calculateHitDepthTimeLimit = (repeat) => 10 + (repeat || 1) * 6;

/** Auto-calculated time limit for CLAP: 10 + repeat * 4 */
export const calculateClapTimeLimit = (repeat) => 10 + (repeat || 3) * 4;

/** Auto-calculated time limit for SPEAK */
export const calculateSpeakTimeLimit = (repeat) => 10 + (repeat || 1) * 15;
...
export function createRESTTask(options = {}) {
    ...
    if (!options.audio) {
        options.audio = 'Rest.REST'
    }

    return {
        ...options,
        type: TaskType.REST,
        desc: options.desc || "rest",
...
export function createHOLDTask(options = {}) {
    ...
    options.audioHalfway = getDefaultHoldAudioHalfway(options.targetDepth)
    options.audioThreeQuarter = getDefaultHoldAudioThreeQuarter(options.targetDepth)
...
/** Speak — phrase recognition; defaults repeat:1, speakMode short, audio Speak.Speak1 */
export function createSPEAKTask(options = {}) {
    if (!options.phrase) {
        options.phrase = 'ready'
    }
    if (!options.repeat) {
        options.repeat = 1
    }
    if (!options.timeLimit) {
        options.timeLimit = calculateSpeakTimeLimit(options.repeat)
    }
    if (!options.speakMode) {
        options.speakMode = SpeakMode.SHORT
    }
    if (!options.audio) {
        options.audio = 'Speak.Speak1'
    }
    const phrase = options.phrase
    const defaultHelp = `Say "${phrase}"`
    return {
        ...options,
        type: TaskType.SPEAK,
        helpText: options.helpText !== undefined ? options.helpText : defaultHelp,
        desc: options.desc || 'speak',
    }
}
...
export const generateRandomTask = () => {
    const taskTypes = [TaskType.HOLDPOSITION, TaskType.UPANDDOWN, TaskType.REST];
...
export const calculateLevelDuration = (tasks, tenSecond = false) => {
    ...
    const totalSeconds = tasks.reduce((sum, task) => {
        ...
        if (task.type === TaskType.HITDEPTH && !taskDuration) {
            taskDuration = calculateHitDepthTimeLimit(task.repeat)
        }
        if (task.type === TaskType.CLAP && !taskDuration) {
            taskDuration = calculateClapTimeLimit(task.repeat)
        }
        if (task.type === TaskType.SPEAK && !taskDuration) {
            taskDuration = calculateSpeakTimeLimit(task.repeat)
        }
...
export const getTaskSummary = (task) => {
    switch (task.type) {
        ...
        case TaskType.SPEAK:
            return `Speak ${task.speakMode === SpeakMode.LONG ? 'long' : 'short'} x${task.repeat}`
        default:
            return task.type
    }
}