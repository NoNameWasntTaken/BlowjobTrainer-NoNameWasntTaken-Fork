...
const AudioCategoryEditor = ({
    ...
    return (
        <div className="audio-category-editor" style={{ marginBottom: '16px', border: '1px solid var(--line)', padding: '12px', borderRadius: '4px' }}>
            <div
            ...
            {expanded && (
                <div style={{ marginTop: '12px' }}>
                    ...
                    {keys.map(key => {
                        ...
                        return (
                            <div key={key} style={{ marginTop: '12px', padding: '8px', backgroundColor: 'var(--surface-soft)', borderRadius: '4px' }}>
                                <button
                                    type="button"
                                    ...
                                    <div id={fileSectionId(key)} className="audio-files-panel">
                                        {dynamicCues && (
                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginBottom: '8px' }}>
                                                ...
                                                {renameErrors[key] && (
                                                    <span style={{ color: 'var(--danger)', fontSize: '0.85em' }}>{renameErrors[key]}</span>
                                                )}
                                            </div>
                                        )}
                                        <AudioFileUploader
                                        ...
                                        />
                                        {files.length === 0 ? (
                                            <div style={{ color: 'var(--muted)', fontStyle: 'italic', marginTop: '8px' }}>No files uploaded</div>
                                        ) : (
                                            <div style={{ marginTop: '8px' }}>
                                                {files.map((filePath, index) => (
                                                    <div key={index} className="audio-file-row">
                                                        ...
                                                        <div className="control-compact" style={{ display: 'flex', gap: '8px' }}>
                                                            <button
                                                                className="button"
                    ...
                    {dynamicCues && (
                        <form onSubmit={handleAddCue} style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', alignItems: 'center', marginTop: '16px' }}>
                            ...
                            {addError && <span style={{ color: 'var(--danger)', fontSize: '0.85em' }}>{addError}</span>}
                        </form>
                    )}
                </div>
            )}
