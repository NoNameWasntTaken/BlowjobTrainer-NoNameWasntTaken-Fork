...
function WebcamDisplay({ webcamRef, canvasRef, canvasDimensions, scaleFactor, fps }) {
    ...
    // Memoize the shaft neighbor map for active grid squares
    const shaftNeighborMap = useMemo(() => {
        const map = new Map()
        const activeSquares = isAllGridsMode ? allGridSquares : (selectedGrid?.squares || [])
        const activeSquaresSet = new Set(activeSquares.map(sq => `${sq.x},${sq.y}`))
        
        for (const { x, y } of activeSquares) {
            const key = `${x},${y}`
            const entry = gridColors[key]
            if (!entry?.shaft) continue

            map.set(key, {
                left: activeSquaresSet.has(`${x - gridSize},${y}`) && gridColors[`${x - gridSize},${y}`]?.shaft,
                right: activeSquaresSet.has(`${x + gridSize},${y}`) && gridColors[`${x + gridSize},${y}`]?.shaft,
                top: activeSquaresSet.has(`${x},${y - gridSize}`) && gridColors[`${x},${y - gridSize}`]?.shaft,
                bottom: activeSquaresSet.has(`${x},${y + gridSize}`) && gridColors[`${x},${y + gridSize}`]?.shaft
            })
        }
        return map
    }, [isAllGridsMode, allGridSquares, selectedGrid, gridColors, gridSize])
