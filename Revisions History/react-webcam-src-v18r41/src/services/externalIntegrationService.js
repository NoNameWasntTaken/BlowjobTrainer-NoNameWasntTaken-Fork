...
let capturesCliEnabledCache = null;
...
async function isCapturesEnabled() {
    if (typeof window === 'undefined' || !window.electronAPI?.isCapturesCLIEnabled) {
        return false;
    }
    if (capturesCliEnabledCache != null) return capturesCliEnabledCache;
    try {
        capturesCliEnabledCache = await window.electronAPI.isCapturesCLIEnabled();
    } catch {
        capturesCliEnabledCache = false;
    }
    return capturesCliEnabledCache;
}
