...
function EndlessDive({ task, onTaskOver }) {
    ...
    const checkScoreEvents = useCallback(() => {
        ...
        const totalScore = totalHoldScoreRef.current + totalDiveScoreRef.current +
        ...
        if (nextThresholdRef.current == null) {
            ...
            nextThresholdRef.current = scoreMin + Math.random() * Math.max(0, scoreMax - scoreMin);
        }
    ...
    // Grace period decay and surface penalty loop
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            ...
            if (diveState.current === 0 && tempGraceRef.current <= 0 && gracePeriodRef.current > 0) {
                gracePeriodRef.current = Math.max(0, gracePeriodRef.current - delta);
            }
            if (diveState.current === 0 && hasStartedRef.current && gracePeriodRef.current <= 0 && tempGraceRef.current <= 0) {
        ...
        // eslint-disable-next-line react-hooks/exhaustive-deps -- diveState ref
    }, []);
