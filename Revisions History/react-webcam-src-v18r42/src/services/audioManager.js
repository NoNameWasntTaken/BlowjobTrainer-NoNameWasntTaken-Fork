...
import { validateCustomCueName } from './customAudio'
...
import { migrateAudioPack, migrateAudioPackFiles, isLegacyCategory } from '../utils/audioPackMigration'
import {
    allocateCanonicalExportPath,
    buildOrderedExportManifest,
    extractBasenameFromStoredPath,
    getExportCategoryOrder,
} from '../utils/audioPackExportUtils'
...
function loadPackFromStorage(packId) {
    const raw = storageService.loadAudioPack(packId)
    return migrateAudioPack(raw).pack
}
...
const loadActivePack = () => {
    ...
    if (activePackId) {
        activeCustomPack = loadPackFromStorage(activePackId)
...
export const audioManager = {
    ...
    getAudioFileForPack: async (audioRef, packId) => {
        const primaryPack = packId ? loadPackFromStorage(packId) : null
    ...
    setFallbackPack: (packId) => {
        if (packId) {
            fallbackPack = loadPackFromStorage(packId)
        } else {
            fallbackPack = null
        }
    },
    ...
    setActiveCustomPack: (packId) => {
        ...
        if (packId) {
            activeCustomPack = loadPackFromStorage(packId)
     ...
     */
    savePack: (packData) => {
        const { pack } = migrateAudioPack(packData)
        return storageService.saveAudioPack({
            ...pack,
            hidden: pack.hidden === true,
        })
    },
     ...
     */
    loadPack: (packId) => {
        return loadPackFromStorage(packId)
    },
    ...
    validateActivePack: () => {
        const activePackId = storageService.getActiveAudioPack()
        if (activePackId) {
            const pack = loadPackFromStorage(activePackId)
            if (!pack) {
    ...
    exportPack: async (packId) => {
        const pack = loadPackFromStorage(packId)
        if (!pack) {
        ...
        const { pack: migratedPack } = migrateAudioPack(pack)
        const audioFiles = migrateAudioPackFiles(migratedPack.audioFiles || {})
        const categoryOrder = getExportCategoryOrder()
        const exportCategories = [
            ...categoryOrder,
            ...Object.keys(audioFiles).filter(
                (c) => !categoryOrder.includes(c) && !isLegacyCategory(c)
            ),
        ]
        const rawManifestAudioFiles = {}
        const usedExportPaths = new Set()
        const extractedToCanonical = new Map()
        const blobCache = new Map()

        for (const category of exportCategories) {
            const keys = audioFiles[category]
            if (!keys) continue

            rawManifestAudioFiles[category] = {}
            ...
            for (const [key, paths] of Object.entries(keys)) {
                ...
                for (const filePath of filePaths) {
                    ...
                    let canonicalPath = extractedToCanonical.get(extractedPath)
                    if (!canonicalPath) {
                        const basename = extractBasenameFromStoredPath(extractedPath)
                        canonicalPath = allocateCanonicalExportPath(category, basename, usedExportPaths)
                        extractedToCanonical.set(extractedPath, canonicalPath)

                        if (!blobCache.has(extractedPath)) {
                            const objectUrl = await audioFileService.getAudioFile(filePath)
                            if (!objectUrl) continue
                            const response = await fetch(objectUrl)
                            blobCache.set(extractedPath, await response.blob())
                        }

                        zip.file(canonicalPath, blobCache.get(extractedPath))
                    }
                ...
                rawManifestAudioFiles[category][key] =
                    filePaths.length === 1 ? manifestPaths[0] : manifestPaths
            }

            if (Object.keys(rawManifestAudioFiles[category]).length === 0) {
                delete rawManifestAudioFiles[category]
            }
        }

        const manifestAudioFiles = buildOrderedExportManifest(rawManifestAudioFiles, categoryOrder)
        ...
        const manifest = {
            ...
            pack: {
                ...
                ...(migratedPack.customNames && Object.keys(migratedPack.customNames).length
                    ? { customNames: migratedPack.customNames }
                    : {}),
                ...(migratedPack.audioRefAliases && Object.keys(migratedPack.audioRefAliases).length
                    ? { audioRefAliases: migratedPack.audioRefAliases }
                    : {}),
                ...(pack.hidden === true ? { hidden: true } : {}),
                ...(pack.elevenLabsVoiceId ? { elevenLabsVoiceId: pack.elevenLabsVoiceId } : {}),
            },
    ...
    importPack: async (zipFile, options = {}) => {
        ...
        const customKeyWarnings = []
        for (const key of Object.keys(processedAudioFiles.Custom || {})) {
            const message = validateCustomCueName(key)
            if (message) customKeyWarnings.push(`Invalid Custom key "${key}": ${message}`)
        }

        const { pack: migratedPack } = migrateAudioPack({
            ...manifest.pack,
            audioFiles: processedAudioFiles,
        })
        const migratedAudioFiles = migratedPack.audioFiles || {}

        // Validate pack structure against default AUDIO
        const warnings = [...customKeyWarnings]
        ...
        const packCategories = Object.keys(migratedAudioFiles)
        ...
        // Check for extra keys within categories. Custom keys are open: only invalid names are warned above.
        packCategories.forEach(cat => {
            if (cat === 'Custom' || !AUDIO[cat]) return
            const defaultKeys = Object.keys(AUDIO[cat])
            const packKeys = Object.keys(migratedAudioFiles[cat] || {})
        ...
        const packData = {
            ...migratedPack,
            audioFiles: migratedAudioFiles,
            hidden: manifest.pack?.hidden === true,
            elevenLabsVoiceId: manifest.pack?.elevenLabsVoiceId || undefined,
        }
