...
function useColorSampling() {
    ...
    const sampleGridSquares = useCallback((squares, canvas, context) => {
        ...
        for (const { x, y } of squares) {
            // Square coords are sensor-buffer pixels (see previewToBufferCoords). The processing
            // canvas is unmirrored; preview mirroring is CSS-only on the video element.
            if (x < 0 || y < 0 || x + 5 > canvas.width || y + 5 > canvas.height) {
                continue
            }

            try {
                // Sample a 5x5 area
                const imageData = context.getImageData(x, y, 5, 5)
