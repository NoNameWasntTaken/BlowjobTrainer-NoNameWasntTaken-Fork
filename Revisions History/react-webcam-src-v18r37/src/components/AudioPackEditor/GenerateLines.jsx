...
function appendAudioFile(prev, category, key, customContentUrl) {
...
const GenerateLines = ({ pack, isEditing, customNames, onUpdateAudioFiles }) => {
    ...
    const commitBlob = useCallback(
        async (category, key, blob) => {
            const { customContentUrl } = await storeGeneratedAudio(pack.id, category, key, blob)
            onUpdateAudioFiles((prev) => appendAudioFile(prev, category, key, customContentUrl))
        },
        ...
        async (jobs) => {
            ...
            await runBatch(jobs, voiceSettings, {
                ...
                onJobSuccess: async (job, blob) => {
                    ...
                    if (!parsed) return
                    try {
                        await commitBlob(parsed.category, parsed.key, blob)
                        updateVariant(parsed.eventId, parsed.variantId, {
                            status: STATUS.DONE,
                            error: null,
                        })
    ...
    const handleGenerateVariant = async (category, key, variantId) => {
        ...
        try {
            ...
            await commitBlob(category, key, blob)
            updateVariant(eventId, variantId, { status: STATUS.DONE, error: null })
        } catch (err) {
            ...
            {currentEventId && currentVariants.length > 0 && (
                <div className="generate-lines-line-editor">
                        ...
                        {currentVariants.map((variant, index) => {
                            ...
                            return (
                                <div key={variant.id} className="generate-lines-variant-row">
                                    ...
                                    />
                                    <div className="generate-lines-variant-actions">
                                        ...
                                        <button
                                            type="button"
                                            className="button"
                                            onClick={() => removeVariant(currentEventId, variant.id)}
                                            disabled={currentVariants.length <= 1 || isRunning}
                    ...
                    <div className="generate-lines-variant-toolbar">
                        ...
                        >
                            Generate all variants ({currentVariantCount})
                        </button>
                    </div>
                </div>
            )}
