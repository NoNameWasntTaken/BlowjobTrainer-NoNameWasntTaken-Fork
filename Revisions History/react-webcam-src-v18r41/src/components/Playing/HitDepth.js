...
function HitDepth({ task, onTaskOver, onSignalCaptureWindow, isLastTask }) {
    ...
    // Skip-feedback handoff: while active, a leftover position from the previous task is
    // parked (reflected but never scored) until the player genuinely resets to the surface,
    // so the task can't auto-complete or auto-fail on carried-over depth.
    const handoffActiveRef = useRef(!!task.skipHandoff)
    ...
    /** True after the player reaches target depth at least once this task (capture window gate). */
    const hasHitTargetOnceRef = useRef(false)
    ...
    useEffect(() => {
        ...
        setHitScores([])
        setHasStarted(false)
        hasHitTargetOnceRef.current = false
        handoffActiveRef.current = !!task.skipHandoff
    }, [task, setFeedback, setSfx])

    // Check if diving has started
    useEffect(() => {
        if (!hasStarted && diveState.current > 0) {
            // Stay unstarted while parked on a handoff, so leftover depth can't trigger
            // surface penalties or failures before the player has actually begun.
    ...
    useEffect(() => {
        ...
        // Skip-feedback handoff: never score carried-over depth. Reflect the leftover
        // position (park at AT_DEPTH if already at/past target) and wait for a genuine reset
        // to the surface, which arms real hit scoring for the first deliberate attempt.
        if (handoffActiveRef.current) {
            ...
            if (depthDifference >= 0) {
                if (hitState !== HitState.AT_DEPTH) setHitState(HitState.AT_DEPTH)
            } else if (hitState !== HitState.NOT_STARTED) {
                setHitState(HitState.NOT_STARTED)
            }
            return
        }

        // Reset for next attempt when shallower than target (depthDifference < 0).
        ...
        // Depth 0 with target > 1 still falls through to hitFailed().
        if (
            hitState === HitState.AT_DEPTH &&
            depthDifference < 0 &&
            (currentDepth !== 0 || targetDepth === 1)
        ) {
        ...
        // (abandoned attempt: not a successful AT_DEPTH reset, handled above)
        ...
        if (hitState === HitState.IN_PROGRESS && depthDifference >= 0) {
            ...
            hasHitTargetOnceRef.current = true
