...
const Training = () => {
    ...
    const handleSelectLevel = async (level) => {
        ...
        // Audio pack override logic for pre-loaded levels
        const isExternalMode = externalIntegrationService.isExternalMode();
        const cliAudioPackId = externalIntegrationService.getAudioPackId();
        
        if (isExternalMode) {
            // Pre-loaded level: Never use Content Library selected pack
            // Priority: 1) Level's audioPackId (if exists and pack found)
            //           2) CLI --audio-pack (if provided)
            //           3) Default pack
            ...
            // Priority 2: CLI --audio-pack (if provided and level pack not found)
            if (!audioPackId && cliAudioPackId) {
                audioPackId = cliAudioPackId;
            }
            
            // Priority 3: Default pack (null = default)
            ...
            if (isDefault) {
                // Default level: Use currently selected pack from Content Library
                // Explicitly sync with storage to ensure activeCustomPack is up-to-date
