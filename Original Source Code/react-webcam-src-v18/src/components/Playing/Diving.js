...
function Diving({ task, onTaskOver }) {
    ...
    const [lastFeedbackType, setLastFeedbackType] = useState(AUDIO.NONE)
    ...
    useEffect(() => {
        ...
        if (task?.type === TaskType.UPANDDOWN) {
            ...
            setFeedback(AUDIO.NONE)
            setSfx(AUDIO.NONE)
            setHasStarted(false)
            setLastFeedbackTime(0) // Reset feedback cooldown
            setLastFeedbackType(AUDIO.NONE) // Reset last feedback type
        }
    ...
    useEffect(() => {
        ...
        if (!isUp && diveState.current === targetDepthDown) {
            setSfx(AUDIO.Sfx.TICK)
        }
        if (isUp && diveState.current === targetDepthUp) {
            setSfx(AUDIO.Sfx.TOCK)
        }
        // if (motion.dir === 'rest') {
        //     setSfx(0)
        // }
    ...
    useEffect(() => {
        ...
        if (motion.dir === 'up') {
            ...
            if (issues.length === 0) {
                ...
                if (Math.random() < 0.7) {
                    provideFeedback(AUDIO.Feedback.PERFECT)
                }
            ...
            if (tempoIssue === DIVE_ERROR.TOO_FAST) {
                provideFeedback(AUDIO.Feedback.TOO_FAST)
            } else if (tempoIssue === DIVE_ERROR.TOO_SLOW) {
                provideFeedback(AUDIO.Feedback.TOO_SLOW)
            } else if (tempoIssue === DIVE_ERROR.WAY_TOO_FAST) {
                provideFeedback(AUDIO.Feedback.WAY_TOO_FAST)
            } else if (tempoIssue === DIVE_ERROR.WAY_TOO_SLOW) {
                provideFeedback(AUDIO.Feedback.WAY_TOO_SLOW)
            }
            // descent issues
            else if (descentIssue === DIVE_ERROR.SHALLOW_DESCENT) {
                provideFeedback(AUDIO.Feedback.SHALLOW_DESCENT)
            } else if (descentIssue === DIVE_ERROR.DEEP_DESCENT) {
                provideFeedback(AUDIO.Feedback.DEEP_DESCENT)
            }
            // ascent issues
            else if (ascentIssue === DIVE_ERROR.SHALLOW_ASCENT) {
                provideFeedback(AUDIO.Feedback.SHALLOW_ASCENT)
            } else if (ascentIssue === DIVE_ERROR.DEEP_ASCENT) {
                provideFeedback(AUDIO.Feedback.DEEP_ASCENT)
            }
            // need to handle - 
    ...
    useEffect(() => {
        if (diveState.current === 0 && motion.dir !== 'rest' && hasStarted) {
            ...
            if (targetDepthDown > 2) {
                setFeedback(AUDIO.Feedback.SURFACE_PENALTY_HARD)
            } else {
                setFeedback(AUDIO.Feedback.SURFACE_PENALTY_SOFT)
            }
