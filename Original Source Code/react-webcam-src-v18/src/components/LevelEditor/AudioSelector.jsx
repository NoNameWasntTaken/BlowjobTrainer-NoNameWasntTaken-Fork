 ...
 */
const AudioSelector = ({ value, onChange, label = "Audio", allowedOptions = null }) => {
    ...
    React.useEffect(() => {
        ...
        // Otherwise, get all available audio options from default and active custom pack
        const defaultAudio = audioManager.getDefaultAudio()
        const customPack = audioManager.getActiveCustomPack()
    ...
    const handlePreview = async () => {
        if (value) {
            const audioPath = audioManager.getAudioFileSync(value)
            if (audioPath) {
