...
function Calibration() {
    ...
    const gridCalibrationDisabled = !cameraEnabled
    ...
    return (
        <React.Fragment>
            <h2 className="tab-title">Grid Calibration</h2>
            {gridCalibrationDisabled && (
                <div className="row-centered margin-y-sm" style={{ color: 'var(--muted)' }}>
                    Camera is off. Enable the camera to calibrate grids.
                </div>
            )}
            <GridSelector disabled={gridCalibrationDisabled} />
            {/* grid brush controls */}
            <div className="row-centered">
                ...
                <button 
                    ...
                    disabled={gridCalibrationDisabled || isAllGridsMode}
                >
                    Clear Grid
                </button>
                <button 
            ...
            <div className="row-centered flex-wrap">
                <ColorReading label="Average Color" color={gridAverageColor && typeof gridAverageColor === 'object' && 'r' in gridAverageColor && !('id' in gridAverageColor) ? gridAverageColor : { r: 0, g: 0, b: 0 }} placeholder={gridCalibrationDisabled} />
                <ColorReading label="Dildo Color" color={baseColor && typeof baseColor === 'object' && 'r' in baseColor && !('id' in baseColor) ? baseColor : { r: 0, g: 0, b: 0 }} placeholder={gridCalibrationDisabled} />
                <ShaftReading label="Dildo (%)" percent={Math.round(gridShaftPercent)} placeholder={gridCalibrationDisabled} />
                <ShaftReading label="Suck Depth" percent={currentState} subtitle="0 to 4" placeholder={gridCalibrationDisabled} />
                {/* <h4><bold>Shaft {Math.round(gridShaftPercent)} %</bold></h4> */}
            </div>
            <BaseColorControls disabled={gridCalibrationDisabled} />
            <SensitivityControls disabled={gridCalibrationDisabled} />
            <PercentControls disabled={gridCalibrationDisabled} />
            <HysterisisControls disabled={gridCalibrationDisabled} />
            <div className="row-centered margin-y margin-y-top">
                <button className={`button padding-x ${showTester ? "button-primary" : ""}`} onClick={toggleTester} disabled={gridCalibrationDisabled}>
            ...
            <p className="help margin-y-sm">
                Beginner steps to set up the above:
            </p>
            <ol>
            ...
            <p className="margin-y-sm">
                ...
                <ul>
                    ...
                    <li>3 - {STR.Depth[3]}</li>
                    <li>4 - {STR.Depth[4]}</li>
                    <li>Balls - Balls region (if configured)</li>
                </ul>
            </p>
            <p className="margin-y-sm">
