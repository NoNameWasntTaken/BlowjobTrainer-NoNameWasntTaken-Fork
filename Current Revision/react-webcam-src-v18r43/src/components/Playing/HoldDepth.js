...
// On a "Skip feedback (fast transition)" handoff, require the player to dwell at the
// target depth this long before the hold officially starts. This stops a leftover stroke
// from the previous task from auto-starting the hold or triggering a surface penalty.
const SKIP_HOLD_DWELL_MS = 400

function HoldDepth({ task, onTaskOver, onSignalCaptureWindow, isLastTask }) {
    ...
    // Skip-feedback handoff: while armed, the hold stays parked (no scoring/penalty) until
    // a brief stable dwell at the target depth. dwellTimerRef holds the pending dwell timer.
    const handoffArmRef = useRef(!!task.skipHandoff)
    ...
    useEffect(() => {
        ...
        const id = setInterval(() => {
            const atCorrect =
                holdState === HoldState.CORRECT_DEPTH && timeTracking.effectiveTime >= 1
            const vidMin = isLastTask ? 6 : 3
            onSignalCaptureWindow(atCorrect, {
                photos: atCorrect,
                videos: atCorrect && timeLeftRef.current >= vidMin,
            })
        }, 1000)
        return () => clearInterval(id)
    }, [onSignalCaptureWindow, holdState, timeTracking.effectiveTime, isLastTask])
    ...
    const holdStart = useCallback(() => {
        ...
        // The hold is now officially underway: disarm the handoff and mark started so
        // surfacing penalties and normal state transitions apply from here on.
        setHasStarted(true)
        handoffArmRef.current = false
    ...
    useEffect(() => {
        if (task?.type === TaskType.HOLDPOSITION) {
            ...
            setHasStarted(false)
            handoffArmRef.current = !!task.skipHandoff
            if (dwellTimerRef.current) {
                clearTimeout(dwellTimerRef.current)
                dwellTimerRef.current = null
            }
    ...
    useEffect(() => {
        if (!hasStarted && diveState.current > 0) {
            // Stay parked until the handoff dwell completes (see hold state tracking).
            if (handoffArmRef.current) return
            setHasStarted(true)
        }
    ...
    useEffect(() => {
        ...
        // Skip-feedback handoff: while armed, ignore all scoring/penalty/transition logic.
        // Only arm a dwell timer once the player is stably at the target depth; if they
        // leave before it fires, cancel it. When it fires, the hold starts normally.
        if (handoffArmRef.current) {
    ...
    }, [task, holdState, holdFailed, holdStart, resetHoldState, setSfx, hasStarted, diveState, holdPenalty])
    ...
    const { barRef } = useHoldProgress(holdState, task?.time, task?.id)
    ...
    useEffect(() => {
        ...
        } else if (!(handoffArmRef.current && holdState === HoldState.NOT_STARTED)) {
            // While parked on a skip-feedback handoff, keep any inherited vibration rather
            // than zeroing it before the hold has actually begun.
            stopVibration()
        }
