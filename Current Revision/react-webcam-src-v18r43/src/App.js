...
import CameraCardHeader from './components/Webcam/CameraCardHeader';
...
function App() {
  ...
  // Splash exit 480ms, then 300ms per segment. Auto-start has no header, so two segments (1080ms) instead of three (1380ms).
  useEffect(() => {
    if (loadingState !== 'exiting') return
    const segments = externalIntegrationService.isAutoStartMode() ? 2 : 3
    const id = setTimeout(() => setLoadingState('ready'), 480 + 300 * segments)
  ...
  useEffect(() => {
    ...
    function finishLoading() {
      ...
      setLoadingState(reduceMotion ? 'ready' : 'exiting')
    }
    ...
    async function initializeExternalMode() {
      try {
        ...
        // In auto-start mode, always enable camera (needed for tracking without visible nav chrome)
        if (externalIntegrationService.isAutoStartMode()) {
          setCameraEnabled(true);
        }

        if (
          externalIntegrationService.isAutoStartMode() &&
          !externalIntegrationService.getLevelId()
        ) {
          setPreflightError(
            '--auto-start requires --level or --level-list (pass a level id with --level <levelId> or --level-list <id1,id2,...>)'
          )
          setLoadingState('error')
          setTimeout(() => {
            if (window.electronAPI?.requestExit) {
              window.electronAPI.requestExit(5, 'auto_start_requires_level')
            }
          }, 3000)
          return
        }
        ...
        if (externalIntegrationService.isCalibrationOnlyMode()) {
          ...
          if (calibrationPath && window.electronAPI && window.electronAPI.readCalibrationFile) {
            ...
            if (result.success && result.data) {
              const importResult = await calibrationService.importCalibration(result.data);
        ...
        if (levelId) {
          ...
          if (calibrationPath && window.electronAPI && window.electronAPI.readCalibrationFile) {
            ...
            if (result.success && result.data) {
              ...
              if (externalIntegrationService.shouldSkipCalibration()) {
                const importResult = await calibrationService.importCalibration(calibrationData);
                if (!importResult.success) {
                  console.error('Failed to import calibration data:', importResult.errors);
                }
              }
          ...
          // Set navigation state based on skip-calibration and auto-start
          ...
          if (skipCalibration) {
            ...
            // In auto-start mode, also set playState to PLAYING to trigger runBeginPlayPipeline from Playing
            if (isAutoStart) {
  ...
  function renderLoadingSplash(exiting) {
    return (
      <div
        className={`container app-shell app-shell--splash${exiting ? ' app-shell--splash-exit' : ''}`}
        aria-hidden={exiting || undefined}
      >
        <main className="content-panel loading-splash">
          <div className="loading-splash__body">
            <h2 aria-live="polite" aria-busy="true" className="loading-ellipsis">
              Loading
              <span className="loading-ellipsis__dots" aria-hidden="true">
                <span className="loading-ellipsis__dot">.</span>
                <span className="loading-ellipsis__dot">.</span>
                <span className="loading-ellipsis__dot">.</span>
              </span>
            </h2>
            {loadingState === 'checking' && <p>Running pre-flight checks...</p>}
            {loadingState === 'loading' && <p>Loading level and configuration...</p>}
          </div>
        </main>
      </div>
    );
  ...
  // Show loading state during initialization. The main shell stays unmounted so preload keeps the thread.
  if (loadingState === 'initializing' || loadingState === 'checking' || loadingState === 'loading') {
    return (
      <div className="App">
        {renderLoadingSplash(false)}
      </div>
    );
  }
  ...
  const hideCameraArea = nav === NAV.ACHIEVEMENTS || nav === NAV.LEVEL_EDITOR ||
    nav === NAV.AUDIO_PACK_EDITOR || (nav === NAV.THEMES && !isExternalMode);
  const showCamera = !hideCameraArea &&
    (nav === NAV.PLAYING || nav === NAV.GAMEOVER || cameraEnabled);
  const showCameraArea = !hideCameraArea;
  const showCameraToggle = nav !== NAV.PLAYING && nav !== NAV.GAMEOVER;

  const shellExiting = loadingState === 'exiting';
  ...
  return (
    <div className="App">
      ...
      <div
        className={`container app-shell${shellExiting ? ' app-shell--enter' : ''}${shellExiting && isAutoStart ? ' app-shell--no-header' : ''}`}
        aria-hidden={shellExiting || undefined}
      >
        {/* Hide navigation in CLI auto-start mode */}
        {!isAutoStart && <Navigation />}
        ...
        {!DEBUG && showCameraArea && (
          <section className="camera-card" aria-label="Camera tracking preview">
            {isAutoStart && <AutoStartCameraChrome />}
            {showCamera ? (
              <WebcamComponent />
            ) : (
              <>
                ...
                {showCameraToggle && (
                  <div className="camera-card-footer camera-card-footer-off">
                    <CameraToggle />
                    <MirrorToggle compact={false} />
                  </div>
                )}
              </>
            )}
      ...
      {shellExiting && renderLoadingSplash(true)}
      {!shellExiting && <Version />}
    </div>
  );
}
