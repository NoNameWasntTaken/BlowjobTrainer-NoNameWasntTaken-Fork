    ...
    for (const task of tasks) {
        switch (task.type) {
            ...
            case TASK_TYPES.ENDLESS:
                ...
                break
            case TASK_TYPES.SPEAK:
                perfectScore += calcPerfectSpeakScore(task)
                break
            default:
                break
        }
...
export function getDifficulty(level) {
    ...
    // Default levels use lookup (no override)
    if (levelManager.isDefaultLevel(level.id)) {
        const lookup = DEFAULT_LEVEL_DIFFICULTIES[level.id]
        if (lookup !== undefined) return lookup
    }

    // Custom levels: use override when enabled and valid
    if (!levelManager.isDefaultLevel(level.id) && level.useDifficultyOverride &&
        typeof level.difficultyOverride === 'number') {
        const v = level.difficultyOverride
        if (v >= 1 && v <= 11) {
            if (v === 11) return 11
            if (v > 10 && v < 11) return 10  // 10.5 invalid, clamp to 10
            return Math.round(v * 2) / 2    // half-step for 1-10
        }
    }
