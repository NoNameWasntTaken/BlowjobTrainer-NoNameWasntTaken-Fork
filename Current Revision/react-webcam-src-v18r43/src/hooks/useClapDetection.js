...
/** Threshold sits on the bar once residual crosses it, so ink is stroked over a surface halo. */
const THRESHOLD_HALO_WIDTH = 3
const THRESHOLD_LINE_WIDTH = 2

function readThemeColor(style, name, fallback) {
    const value = style.getPropertyValue(name).trim()
    return value || fallback
}

/** Canvas fill/stroke ignore `var(--…)`; resolve tokens from the element. */
function drawClapMeter(canvas, barFrac, thrFrac) {
    const ctx2d = canvas.getContext('2d')
    const style = getComputedStyle(canvas)
    const accent = readThemeColor(style, '--accent', '#2563eb')
    const ink = readThemeColor(style, '--ink', '#0f172a')
    const surface = readThemeColor(style, '--surface', '#ffffff')

    ctx2d.clearRect(0, 0, canvas.width, canvas.height)
    const height = Math.min(1, Math.max(0, barFrac)) * canvas.height
    ctx2d.fillStyle = accent
    ctx2d.fillRect(0, canvas.height - height, canvas.width, height)

    const y = canvas.height - Math.min(1, Math.max(0, thrFrac)) * canvas.height
    ctx2d.beginPath()
    ctx2d.moveTo(0, y)
    ctx2d.lineTo(canvas.width, y)
    ctx2d.lineWidth = THRESHOLD_HALO_WIDTH
    ctx2d.strokeStyle = surface
    ctx2d.stroke()
    ctx2d.lineWidth = THRESHOLD_LINE_WIDTH
...
export function useClapDetection(onClap, _isCalibration = false, micEnabled = true, micLifecycle = 'own') {
    ...
    /** Gameplay: count claps only when instruction lifecycle has reached `ready` (blocks idle/pending/playing/cooldown). */
    const instructionBlockingClapsRef = useRef(false)
    instructionBlockingClapsRef.current =
        micLifecycle === 'gameplay' && instructionPhase !== 'ready'
        ...
        const runDetectLoop = (analyser) => {
            ...
            const detectSound = () => {
                ...
                if (canvasRef.current) {
                    ...
                    drawClapMeter(canvasRef.current, barFrac, thrFrac)
                }
    ...
    useEffect(() => {
        ...
        const drawIdleThreshold = () => {
            const residualThr = currentThreshold * RESIDUAL_THRESHOLD_SCALE
            const thrFrac = Math.min(1, residualThr / RESIDUAL_VISUAL_CAP)
            drawClapMeter(canvas, 0, thrFrac)
        }
        drawIdleThreshold()
        const observer = new MutationObserver(drawIdleThreshold)
